...
A thought with `pending: true` is not fully in memory. Either the thought itself has not been loaded yet — the three root thoughts start this way in [`initialState`](../src/util/initialState.ts) to trigger the first pull, and [`getContextsSortedAndRanked`](../src/selectors/getContextsSortedAndRanked.ts) stands in a placeholder for a context that has not arrived — or the thought is loaded but some of the children in its `childrenMap` are not in `thoughtIndex`. The second state is normal rather than corrupt: `fetchDescendants` stops fetching at the [buffer depth](persistence.md#fetchdescendants-the-actual-pull-engine), and [`freeThoughts`](../src/actions/freeThoughts.ts) frees thoughts that have gone off-screen by deleting them from the index with `local: false, remote: false`, which leaves the parent's `childrenMap` intact and marks the parent pending. The map keeps naming the absent children so that the pull queue ([`pullQueue.ts`](../src/redux-middleware/pullQueue.ts)), which drives fetches based on visible pending IDs, can restore them once the parent is visible again. Selectors resolve children through [`childIdsToThoughts`](../src/selectors/childIdsToThoughts.ts), which skips the ones not in memory, and [`mergeUpdates`](../src/util/mergeUpdates.ts) clears the flag once every child in the map is present — so a reducer that drops the absent entries from a parent's `childrenMap` hides those children until the next reload. The UI renders pending thoughts with placeholders. See [persistence.md](persistence.md).
...
- Thoughts created afterwards are given a rank that keeps the context sorted, via [`getSortedRank`](../src/selectors/getSortedRank.ts) — a fractional rank between the neighbors the new value sorts between. Under `Created` a thought created in the same millisecond as its siblings is still the newest of them, so an ascending context ranks it after them; this is what keeps the sentences of a split thought, which are all created within one millisecond, in order.
- An edit re-ranks the thought only when it changes the thought's sort key: under `Alphabetical`, where the key is the value, and under `Updated`, where the edit moves `lastUpdated` to now. A `Created` context leaves the edited thought's rank alone, since editing does not change when the thought was created.
