...
- Thoughts created or edited afterwards are given a rank that keeps the context sorted, via [`getSortedRank`](../src/selectors/getSortedRank.ts) — a fractional rank between the neighbors the new value sorts between. That rank alone reorders the thought on screen but not in storage, which keeps sibling order structurally, so [`editThought`](../src/actions/editThought.ts) also names the sibling to place it after — see [persistence.md → Order and placement](persistence.md#order-and-placement). A newly created thought needs no such placement, since the insert path derives one from its rank.
