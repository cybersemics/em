...
- [`/src/device`](../src/device) — Device/DOM-level helpers for selection, scrolling, clipboard, focus, and platform detection. The selection wrapper [`device/selection.ts`](../src/device/selection.ts) is the single point of access to `window.getSelection()` (enforced by lint). [`device/deferredHtml.ts`](../src/device/deferredHtml.ts) tracks editables whose canonical HTML is intentionally deferred while Android owns a native selection range. See [cursor-and-caret.md](cursor-and-caret.md).
