...
- [`/src/stores`](../src/stores) — Lightweight non-Redux ministores for ephemeral UI state. Examples: [`editingValue`](../src/stores/editingValue.ts) (the in-progress thought text), [`viewport`](../src/stores/viewport.ts), [`scrollTop`](../src/stores/scrollTop.ts), [`gesture`](../src/stores/gesture.ts), [`syncStatus`](../src/stores/syncStatus.ts), [`selectionRangeStore`](../src/stores/selectionRangeStore.ts). Also home to the mutable flags that must stay out of Redux for performance and used to live in a globals module — [`touch`](../src/stores/touch.ts) (`touching`, `suppressCursorAfterTouch`), [`editableSync`](../src/stores/editableSync.ts) (`suppressChange`, `suppressBlurSync`), [`heldKeys`](../src/stores/heldKeys.ts) (`suppressExpansion`, `arrowKeyBoundaryCross`), [`nativeHistoryGesture`](../src/stores/nativeHistoryGesture.ts), [`freeThoughtsThreshold`](../src/stores/freeThoughtsThreshold.ts), and [`abandonImport`](../src/stores/abandonImport.ts). A `getState()` read subscribes to nothing, so writing one never re-renders, and `resetStores` clears them between tests.
