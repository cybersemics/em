...
- **Native platform projects** — `android/`, `ios/`, `desktop/`, and `assets/` (the icon and splash source layers, at the root of `yarn icons`).

A change set confined to those paths cannot affect what any of the four workflows tests: `yarn build` is web-only (`build:packages`, `build:styles`, `vite build`), `yarn test` reads only `src/`, and BrowserStack exercises mobile Safari over a tunnel rather than the Capacitor app. None of the native directories contains a JS or TS file. `assets/` also feeds `public/`'s Apple touch icon and Android/PWA icons — `yarn icons:web` renders them from `assets/icon-only.png` — but that rendering is a manual, committed step (`scripts/generate-web-icons.mjs`), never something `vite build` runs itself; editing `assets/` alone leaves the committed `public/` output, and so the shipped build, untouched. `public/favicon.ico`, `favicon-16x16.png`, and `favicon-32x32.png` are hand-maintained and outside this pipeline entirely. A real icon change always regenerates and commits that `public/` output alongside the `assets/` change, and `public/` carries no filter, so the workflows still run. **If icon or splash generation is ever folded into `yarn build` itself, the native entries must be removed** — otherwise a real change would ship untested.
