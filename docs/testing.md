...
Shared setup dismisses the visible welcome modal and starts interacting without waiting for the local database. [`resetApp`](../src/e2e/puppeteer/helpers/resetApp.ts) likewise waits for the visible empty thoughtspace, not database readiness. The frontend must remain usable while persistence initializes in the background; late cursor restoration must preserve a cursor already established by interaction. A blanket initialization wait would hide regressions in that contract. [`startup.ts`](../src/e2e/puppeteer/__tests__/startup.ts) uses `reloadWithDeferredInitialization` to hold initialization, interact, release the real initializer, and assert that typing and note focus survive its completion. Persistence-specific reloads through [`refresh`](../src/e2e/puppeteer/helpers/refresh.ts) still synchronize pending writes and initialization; tests should otherwise prefer a waiter for the expected visible content or cursor.
...
| Non-visual synchronization | Wait | [`waitForThoughtspaceIdle`](../src/e2e/puppeteer/helpers/waitForThoughtspaceIdle.ts), [`waitForInitialized`](../src/e2e/puppeteer/helpers/waitForInitialized.ts) | Use only when persistence or another prerequisite has no immediate visual signal. `waitForThoughtspaceIdle` waits for every queued persistence write to commit and rejects if one failed; call it only to synchronize before a reload or before asserting that a write landed. `waitForInitialized` is retained for persistence-specific loading, including `refresh`, not as a general UI-readiness condition. Shared setup and `resetApp` deliberately do not call it. Assert the final user-visible result separately. |
| Controlled background startup | Arrange and release | [`reloadWithDeferredInitialization`](../src/e2e/puppeteer/helpers/reloadWithDeferredInitialization.ts) | Uses the existing `preventInitialize` flag to make interaction before database initialization deterministic. Its returned, idempotent release runs the real initializer, not a mock. Release in the test body and assert visible interaction survives completion; call release again in finally for cleanup on failure. Per-test page isolation removes the preload. Never use this control to make ordinary tests wait for database readiness. |
