...
| Tap a thought by value | [`tapThought`](../src/e2e/puppeteer/helpers/tapThought.ts) | Resolves the semantic target and taps it with a touch pointer. `clickThought` dispatches a DOM click instead, which does not reach the touch input path, so a thought tapped after a touch drag is not activated by it. |
...
| Drag and drop with touch | [`dragAndDropThoughtTouch`](../src/e2e/puppeteer/helpers/dragAndDropThoughtTouch.ts) | Long presses the source thought until its bullet reports that react-dnd's TouchBackend has begun the drag, drags to the destination, and releases. The mouse helpers only exercise the HTML5 backend used on desktop, so a drag whose behavior differs on mobile needs this one under device emulation. |
