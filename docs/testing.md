...
The choice belongs inside the waiter helper, not at the call site. A helper that waits for a user-visible outcome — [`waitForCursor`](../src/e2e/puppeteer/helpers/waitForCursor.ts), [`waitForAlert`](../src/e2e/puppeteer/helpers/waitForAlert.ts), [`waitForCommandCenterOpen`](../src/e2e/puppeteer/helpers/waitForCommandCenterOpen.ts) — polls internally and reports what it saw (the thought the cursor actually landed on, the alert that rendered instead, the sheet state it is stuck in), so tests keep calling it by name whether they are arranging or asserting. Do not hand-roll that reporting by catching a wait's timeout and re-reading the DOM to build a message; that is what `expect.poll` already does.
