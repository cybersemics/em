...
- [`initStore`](../src/test-helpers/initStore.ts) — async store setup without mounting the React tree. Clears Redux state, resets ministores via `resetStores`, reinitializes the in-memory thoughtspace with the app's committed-publication bridge, and enables fake timers. Await it (or pass it directly to `beforeEach`).
...
The AI workspace uses the same `typescript` → `@typescript/typescript6` alias as the root: Vercel's backend type checker needs the JavaScript compiler API, which TypeScript 7's native package does not provide.
