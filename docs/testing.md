...
| Temporary #5305 diagnostics | Observe | [`captureGestureDiagnostics`](../src/e2e/puppeteer/helpers/captureGestureDiagnostics.ts) | Enabled only by `EM_GESTURE_DIAGNOSTICS=1` for the unmounted-touch-target investigation. Starts browser input/compositor tracing before page creation; records existing gesture console logs and pre/post-test viewport, popup geometry, and limited state. Adds no DOM input listeners and changes no input steps, assertions, viewport settings, or timeouts. Restores the diagnostic logging flag afterward. The Puppeteer workflow uploads `gesture-unmount-diagnostics` on success or failure. Remove the helper, hooks, workflow wiring, and this entry after the investigation; observer overhead can affect timing, so passing traces are controls, not proof of a fix. |
