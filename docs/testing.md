...
Coordinates in the iOS suite are screen-relative, not page-relative. Appium's XCUITest driver always executes `performActions` in the native context, even while the WEBVIEW context is active, so any coordinate handed to it is measured from the top-left of the device screen. Convert with [`getScreenOffsetY`](../src/e2e/iOS/helpers/getScreenOffsetY.ts), which [`tap`](../src/e2e/iOS/helpers/tap.ts) applies for you. Never compensate for the gap with a hand-tuned `y` offset at the call site.

Two traps sit behind that helper. The offset cannot be read off the accessibility tree: `//XCUIElementTypeOther[@name="em"]` looks like it gives the browser chrome's height, and [`getElementRectByScreen`](../src/e2e/iOS/helpers/getElementRectByScreen.ts) assumes so, but it has been observed reporting 58, 32, 30, 28, 26 and -31 for the same inset in a single run — so `getScreenOffsetY` measures the offset instead, by aiming a tap at a known screen point and asking the page which `clientY` it received. And the offset applies to *viewport* coordinates only: `browser.getElementRect` is page-relative, which for a `position: fixed` element such as the toolbar makes its y the scroll offset, since the W3C algorithm adds the scroll to a bounding rect pinned at 0. Measure with `getBoundingClientRect`.
