...
**Element rects are viewport coordinates, but `performActions` delivers touches in screen coordinates.** The difference is the Safari chrome above the page — [`SAFARI_CHROME_TOP`](../src/e2e/iOS/helpers/safariChromeTop.ts), measured at 60px and constant across keyboard states. `tap` adds it, so its `y` option means an offset *beyond* the element (to aim at the empty space below it, say) rather than the conversion itself. [`getElementRectByScreen`](../src/e2e/iOS/helpers/getElementRectByScreen.ts) adds it too, for the raw `performActions` and `gesture` calls that need screen coordinates directly. Never apply it twice.

Two things move a rect out from under a touch aimed with it. Opening or closing the keyboard scrolls the page, so a rect read on one side of that transition does not locate the element on the other — read it again after the keyboard changes. And `window.scrollTo(0, 0)` while the keyboard is up desynchronises the document scroll from the visual viewport, which Safari otherwise keeps in step; the conversion is then wrong by `visualViewport.offsetTop` and the touch lands that far below its target. Nothing asserts either of these, so treat them as measurements rather than guarantees — the symptom is a touch that quietly lands a row away from where it was aimed.
...
A second value in a dropdown that is already open is tapped directly, since calling `tapToolbar` again would tap its button and toggle the dropdown closed. Tap it with [`tap`](../src/e2e/iOS/helpers/tap.ts) and the `toolbarTapOptions` exported by `tapToolbar`, so it uses the same touch pointer.
