...
Puppeteer setup preloads `testFlags.thoughtspaceStorage` before the application starts. Most browser tests use isolated in-memory storage for speed; persistence-specific suites opt into OPFS with `usePersistentTreecrdtStorage`. The application entry point passes that storage choice to `initialize`, so tests exercise the production startup path without a separate data-provider bootstrap configuration.
