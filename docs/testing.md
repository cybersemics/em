...
Per-feature waiters include [`waitForEditable`](../src/e2e/puppeteer/helpers/waitForEditable.ts), [`waitForCursor`](../src/e2e/puppeteer/helpers/waitForCursor.ts), [`waitForAlertContent`](../src/e2e/puppeteer/helpers/waitForAlertContent.ts), [`waitForContextHasChildWithValue`](../src/e2e/puppeteer/helpers/waitForContextHasChildWithValue.ts), and [`waitForThoughtExistInDb`](../src/e2e/puppeteer/helpers/waitForThoughtExistInDb.ts). Every Puppeteer test should read as a sequence of these helpers.
...
Coordinates in the iOS suite are screen-relative, not page-relative. Appium's XCUITest driver always executes `performActions` in the native context, even while the WEBVIEW context is active, so any coordinate handed to it is measured from the top-left of the device screen rather than the top-left of the page — the two differ by the browser chrome above the web content. Bridge the frames with [`getScreenOffset`](../src/e2e/iOS/helpers/getScreenOffset.ts), which returns the offset to add to a viewport coordinate (`getBoundingClientRect`, `Range` rects) along with the page scroll to subtract from a page coordinate (`browser.getElementRect`, whose W3C algorithm adds the window scroll). [`getElementRectByScreen`](../src/e2e/iOS/helpers/getElementRectByScreen.ts) applies both to an element rect and is what [`tap`](../src/e2e/iOS/helpers/tap.ts) measures with. Do not hand `performActions` a raw DOM measurement, and do not compensate with a hand-tuned `y` offset at the call site: the inset is not constant, since Safari's top bar collapses as the page scrolls. `getScreenOffset` caches the native measurement — which costs two context switches and an XCUITest XPath query — against the web viewport metrics, which grow by exactly what the chrome gives up, so it re-measures when and only when the chrome moves.
