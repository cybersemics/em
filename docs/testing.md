...
Coordinates in the iOS suite are screen-relative, not page-relative. Appium's XCUITest driver always executes `performActions` in the native context, even while the WEBVIEW context is active, so any coordinate handed to it is measured from the top-left of the device screen rather than the top-left of the page. Measure with [`getElementRectByScreen`](../src/e2e/iOS/helpers/getElementRectByScreen.ts), which adds the web content's native origin to the DOM rect, and which [`tap`](../src/e2e/iOS/helpers/tap.ts) uses. Never hand `performActions` a raw DOM measurement, and never compensate for the gap with a hand-tuned `y` offset at the call site: the inset is not a constant, because the native origin moves as Safari's top bar collapses and as the page scrolls, so it has to be measured per call.
