...
One narrow gap is worth knowing about. `yarn test` is `vitest --project unit`, whose `**/__tests__/**/*.ts` glob also collects the workspaces under `scripts/`, and the milestone categorizer's sample-integrity tests ([`scripts/milestone/src/__tests__/samples.ts`](../scripts/milestone/src/__tests__/samples.ts)) read the prompt and samples they guard as fixtures. Those assets live beside the code in `scripts/milestone/`, so a sample edit is a `.json` change that runs Test normally — but the prompt itself is `scripts/milestone/instructions.md`, and `**/*.md` is filtered. A pull request that edits only the prompt therefore skips the checks on it, so run `yarn test` locally when editing it.
