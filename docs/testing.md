...
A native overlay such as the magnifier is not assertable in the accessibility tree, so assert its **effect** — the caret and selection, read back from the web context. State that lasts only as long as the gesture needs to be recorded rather than sampled: `performActions` blocks until the finger lifts, by which time a press has ended and `state.longPress` is back to `Inactive`. [`observeDragActivation`](../src/e2e/iOS/helpers/observeDragActivation.ts) installs a `MutationObserver` on the body beforehand and latches `data-drag-hold` / `data-drag-in-progress`, the same attributes that drive the bullet highlight and the drag alert.
