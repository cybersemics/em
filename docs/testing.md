...
**Element rects are viewport coordinates, but `performActions` delivers touches in screen coordinates.** [`getScreenOffsetY`](../src/e2e/iOS/helpers/getScreenOffsetY.ts) returns the difference for the current session: 60px of Safari chrome in mobile Safari (measured, and constant across keyboard states), and 0 in the Capacitor app, whose web view starts at the top of the screen. The same helpers drive both — the wdio suite runs Safari, the `browser-control-ios` bridge runs the app — so the platform is read from the page rather than assumed. `tap` adds the offset, so its `y` option means an offset *beyond* the element (to aim at the empty space below it, say) rather than the conversion itself. [`getElementRectByScreen`](../src/e2e/iOS/helpers/getElementRectByScreen.ts) adds it too, for the raw `performActions` and `gesture` calls that need screen coordinates directly. Never apply it twice.
