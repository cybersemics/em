...
`initStore` is async and enables fake timers. By default, it drops and reinitializes the in-memory TreeCRDT thoughtspace, clears the shared Redux store, and resets every [ministore](glossary.md#m) to its initial state. Ministores are module-level singletons that Vitest isolates per test file, not per test. The same is true of the module-scope `throttle`/`debounce` wrappers (`saveCursor`, `saveJumpHistory`, `updateScrollTop`, …): each is registered with [`cancelOnReset`](../src/util/cancelOnReset.ts), and `resetStores` cancels their pending trailing calls before it resets the stores, so a call scheduled in one test cannot write a stale value back after the reset. A global `afterEach` in [`setupTests.ts`](../src/setupTests.ts) cancels them again at the end of every test, which is what keeps a trailing call from firing into teardown after the last test in a file (#3345); `cleanupTestApp` cancels them before it clears storage for the same reason. `initStore({ persist: true })` skips the thoughtspace, Redux, and ministore resets. Pass it directly to `beforeEach(initStore)` so Vitest awaits it; wrappers must explicitly `await initStore()`. `createTestApp` resets ministores before `initialize({ storage: 'memory' })` runs, and additionally mounts the React tree, initializes persistence and event handlers, and enables the test drag-and-drop backend. `cleanupTestApp` clears storage, the TreeCRDT thoughtspace, the store, and event handlers, and flushes pending timers. Do not share fixture state between tests or rely on test execution order.
...
- [`initStore`](../src/test-helpers/initStore.ts) — async store setup without mounting the React tree. Clears Redux state, resets ministores and cancels pending module-scope throttles via `resetStores`, reinitializes the in-memory thoughtspace, and enables fake timers. Await it (or pass it directly to `beforeEach`).
