...
- **In-process — the slot wait.** `wdio.browserstack.conf.ts`'s `onPrepare` calls `waitForBrowserStackSlots(maxInstances)` after claiming a tunnel and before any session is created. It polls `https://api.browserstack.com/automate/plan.json` (Basic auth with `BROWSERSTACK_USERNAME` / `BROWSERSTACK_ACCESS_KEY`) every ~15s with jitter, logging usage as it waits, until there is both parallel headroom (`parallel_sessions_max_allowed - parallel_sessions_running >= maxInstances`) **and** queue headroom (`queued_sessions + maxInstances <= queued_sessions_max_allowed`). The queue is a separate cap: extra `POST .../session` calls wait there when all parallels are busy, and overflowing it fails with `BROWSERSTACK_QUEUE_SIZE_EXCEEDED` even when parallels look free. After 45 minutes it throws an error naming the observed usage, and the config's `catch` kills the tunnel connector rather than starting workers against a half-claimed tunnel. `maxInstances` stays at 2 so one run does not take the whole parallel cap and a handful of overlapping CI jobs cannot burst 4×5 session creates into that queue.
...
**Who contends for the cap.** Everything that runs iOS Safari through this config draws on the **Automate** product: `ios.yml`, `tdd.yml`'s iOS job (a separate concurrency group, and it inherits the same wait), and local or agent `yarn test:ios`. Agent iOS sessions started by [`start-ios-session.mjs`](../scripts/start-ios-session.mjs) use **App Automate**, which has its own counter: measured against the account, one live Automate session moved `automate/plan.json`'s `parallel_sessions_running` to 1 while `app-automate/plan.json` still reported 0, and each product allows 5. The waiter therefore reads the Automate endpoint alone — summing both would make CI wait for headroom it does not need.

The wait is a check-then-create, not a reservation: two runs can see the same headroom in the same instant. Requiring queue room stops a second wave from starting while the first wave's session creates are already queued; it cannot stop four jobs that all observe an idle plan in the same poll. `specFileRetriesDeferred` in [`wdio.base.conf.ts`](../src/e2e/iOS/config/wdio.base.conf.ts) remains the fallback for that race, re-queueing a spec that failed to acquire a session at the end of the run.

Two consequences of a run starting late are handled inside the `run` job — a job-level `if:` cannot handle either, because it is evaluated when the run is *created*, while the pull request is still open:

- **Merged or closed PRs cancel themselves.** The first step re-reads the pull request state and cancels its own run via the API if the PR is no longer open — a merge triggers the workflow's own `push` run on `main`, so re-testing the merged code would prove nothing while holding a device session for a full suite. Cancelling rather than exiting green is deliberate: no test ran, so nothing may report as passed.
