...
Shared setup waits for app initialization before it dismisses the welcome modal, and [`refresh`](../src/e2e/puppeteer/helpers/refresh.ts) and [`resetApp`](../src/e2e/puppeteer/helpers/resetApp.ts) re-establish the same barrier after a reload, all through [`waitForInitialized`](../src/e2e/puppeteer/helpers/waitForInitialized.ts). The welcome modal is not a readiness signal: it renders from initial state while `initialize()` is still loading the thoughtspace, and dismissing it makes the app fully interactive. Initialization then ends by restoring the cursor from the URL (`initializeCursor`), so a test that had already created a thought, focused a note, or selected thoughts before that point had its state overwritten — typed text landed in the previous thought, a note lost focus, or the first selected thought dropped out of a multiselection. The window was small with the previous storage layer but grew with TreeCRDT's longer startup, so the failures surfaced as intermittent desktop timeouts and content-corrupted snapshots. Tests therefore start against a fully initialized app; only [`startup.ts`](../src/e2e/puppeteer/__tests__/startup.ts), which tests behavior during initialization, opts out with `preventInitialize`.
