...
`initStore` and `createTestApp` enable fake timers because database replication and throttled/debounced side effects may otherwise continue after a test and access a torn-down environment. When a test calls `initialize()` or performs database work directly, explicitly flush the resulting scheduled work before asserting:
