...
`wdio.browserstack.conf.ts` pins a single capability — **iPhone 15 Pro Max on iOS 26** — so every spec runs on that device. **Keep the OS current when BrowserStack offers a newer one.** A suite that exists to catch device-specific WebKit behavior reports green on a WebKit old enough to predate the bug under test: the [#4263](https://github.com/cybersemics/em/issues/4263) `Popover` relayout grows the toolbar by 11.6px on iOS 26 and does not reproduce at all on iOS 17, so the regression test for it passed on the base branch while pinned there and TDD correctly flagged it as covering nothing. Changing the device is not free — the suite converts page coordinates to screen coordinates with a fixed Safari-chrome offset (`toolbarTapOptions` in [`tapToolbar`](../src/e2e/iOS/helpers/tapToolbar.ts)) and matches native elements by name, both of which can move between iOS versions — so a bump is validated by a full [`ios.yml`](../.github/workflows/ios.yml) run, not by the changed spec alone. Hold the screen geometry across a bump where possible, so that only the OS varies. The local Simulator config is pinned separately and is not a substitute: a simulator does not reproduce every device behavior (see [Cursor and Caret](cursor-and-caret.md)).
