...
The wait is a check-then-create, not a reservation: two runs can see the same headroom in the same instant. `specFileRetriesDeferred` in [`wdio.base.conf.ts`](../src/e2e/iOS/config/wdio.base.conf.ts) remains the fallback for that race, re-queueing a spec that failed to acquire a session at the end of the run.
