...
Two things move a rect out from under a touch aimed with it. Opening or closing the keyboard scrolls the page, so a rect read on one side of that transition does not locate the element on the other — read it again after the keyboard changes. And `window.scrollTo(0, 0)` while the keyboard is up desynchronises the document scroll from the visual viewport, which Safari otherwise keeps in step; the conversion is then wrong by `visualViewport.offsetTop` and the touch lands that far below its target. [`viewportCoordinates.ts`](../src/e2e/iOS/__tests__/viewportCoordinates.ts) is the controlled pair demonstrating both halves — the same aim, with and without the scroll.
