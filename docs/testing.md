...
- **Wait for a precondition.** When the line only brings the app to the state a later assertion is about, `page.waitForFunction` (or `waitUntil`) is right: the predicate is evaluated in the page, which is cheaper than a round trip per poll, and a timeout there means the arrange never completed.
...
The two poll in different places, which is what makes the split a performance question as well as a diagnostic one. `page.waitForFunction` compiles the predicate into the page and runs it there on every animation frame, so a wait of any length costs the same handful of protocol messages. `expect.poll` runs its function in node and re-reads the page each attempt: the first attempt is immediate, so a condition that is already true costs one round trip and no added time, but a condition that is not yet true is only noticed at the next attempt. The `puppeteer-e2e` project sets `expect.poll` to a 16 ms interval and a 6 s timeout for this reason — the interval matches an animation frame so the granularity is the same either way, and the messages ride along with the wait rather than adding to it. What this does not make free is polling a condition that is nearly always already true: `waitForEditable` alone is called 139 times, and paying a round trip per call to improve a message that its callers do not read is not a trade worth making.

The choice belongs inside the waiter helper, not at the call site. A helper that waits for a user-visible outcome — [`waitForCursor`](../src/e2e/puppeteer/helpers/waitForCursor.ts), [`waitForAlertContent`](../src/e2e/puppeteer/helpers/waitForAlertContent.ts), [`waitForCommandCenterOpen`](../src/e2e/puppeteer/helpers/waitForCommandCenterOpen.ts) — polls internally and reports what it saw (the thought the cursor actually landed on, the alert that rendered instead, the sheet state it is stuck in), so tests keep calling it by name whether they are arranging or asserting. Do not hand-roll that reporting by catching a wait's timeout and re-reading the DOM to build a message; that is what `expect.poll` already does.
