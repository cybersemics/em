...
A useful invariant falls out of this: **the trigger under test appears exactly once — in the act.** When a command is needed incidentally in some other test's arrange, seed the state it produces with `paste` if that state is an attribute — [table-view.ts](../src/e2e/puppeteer/__tests__/table-view.ts) carries `=view`/`Table` in its fixture rather than toggling Table View. Otherwise trigger the command through its most stable real entry point, a long-standing keyboard shortcut or toolbar button: [multiselect.ts](../src/e2e/puppeteer/__tests__/multiselect.ts) tests *copy*, so it arranges the selection with Select All's `press('a', { meta: true, alt: true })` and acts with a real `press('c', { meta: true })`. When coupling to a shortcut is a concern, run the command through the Command Universe with [`command(label, { inputType: 'commandPalette' })`](../src/e2e/puppeteer/helpers/command.ts), which is keyed by the command's label. The trade-off is accepted knowingly: if the Select All shortcut changes, the tests that arrange with it fail alongside the Select All test. A backdoor that executes the command by id would avoid that, but it skips what every real entry point does — the `commandEmitter` trigger that flushes the pending throttled edit, and the multicursor iteration of `executeCommandWithMulticursor` — so the state it arranges is not one a user can reach.
...
3. Command needed **incidentally** in another test's arrange? Seed the resulting state with `paste` when it is an attribute (e.g. `=view`/`Table`); otherwise trigger the command through its most stable real entry point — a long-standing keyboard shortcut or toolbar button — or through the Command Universe with `command(label, { inputType: 'commandPalette' })` when coupling to a shortcut is a concern. A shortcut change then fails the tests that arrange with it too; that is the price of an arrange that reaches the same state a user does.
...
| Incidental app setup | Arrange | [`openModal`](../src/e2e/puppeteer/helpers/openModal.ts), [`setTheme`](../src/e2e/puppeteer/helpers/setTheme.ts) | Use only when the modal entry point or Settings navigation is not under test. A command needed incidentally has no backdoor: seed its state with `paste` or trigger it through a real entry point ([Principle 2](#2-arrange-with-shortcuts-act-as-the-user-assert-on-what-the-user-sees)). |
...
10. **Scope and coupling** — Does the test cover one independently diagnosable behavior? Is its trigger used exactly once, with incidental commands arranged through `paste` or a real entry point rather than executed by id?
...
| Run a command from the Command Universe | [`command`](../src/e2e/puppeteer/helpers/command.ts) | Opens the desktop Command Universe with Cmd+P, types the command's label — typed as [`CommandLabel`](../src/@types/CommandLabel.ts) — and presses Enter, e.g. `command('Extract Subthought', { inputType: 'commandPalette' })`. |
