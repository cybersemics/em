...
| Emulate a mobile device | [`deviceEmulation.useForSuite`](../src/e2e/puppeteer/helpers/deviceEmulation.ts), [`emulate`](../src/e2e/puppeteer/helpers/emulate.ts) | `useForSuite` applies a Puppeteer device profile before navigation and is preferred. `emulate` applies one to an already mounted app, which reloads the page, so it waits for app initialization to finish again. |
