...
| Browser work | Yes — provisioned Chrome and real iPhones | Chrome only, and only once you start it yourself — never iPhones |
...
A Claude Code cloud session has neither the Copilot tools nor a `gh` binary, but it does have a GitHub MCP server, so the MCP-first, `gh`-second shape those clauses already take is the one to follow there — reaching for the MCP branch rather than the command.
...
These depend on things the runner provides: Chrome already listening on a debugging port, a dev server already up, BrowserStack credentials, and MCP servers configured outside this repository. What they depend on is the *provisioning*, though, not the cloud — a Claude Code cloud session brings up Chrome and the Docker harness itself and runs `run-test`'s web/android half unchanged ([Claude Code in the cloud](#claude-code-in-the-cloud)), so outside the iOS half, where the credentials are genuinely missing, the remaining barrier is the MCP wiring rather than the resources. The `reproduce` and `run-test` skills are not conceptually cloud-only — reproduce before theorising, and never let a skipped test's "0 tests run" masquerade as a pass, are good rules anywhere — but both delegate to `browser-control`, so adapting them means solving the local browser story first. `AGENTS.md` states the reproduce-first principle in prose instead, so the discipline survives even though the skill does not.
...
"Local" above means *not the Copilot cloud agent*, which is not quite the same as *on a laptop*: a Claude Code session started from [claude.ai/code](https://claude.ai/code) reads `AGENTS.md` through the same `CLAUDE.md` symlink and the same `.agents/skills/`, but runs on a disposable Anthropic-managed runner rather than on a developer's machine. Everything above still applies to it, with one correction to the table's browser row: browser work *is* available there, even though nothing is provisioned for it.

Docker runs there once `dockerd` is started by hand, browserless serves CDP out of it, and the whole puppeteer suite passes — image snapshots included, so visual regression work is viable and not only behavioural work. Puppeteer's own downloaded Chrome works too: every step of the [`agent-scripts.yml`](../../.github/workflows/agent-scripts.yml) job passes in-session, including [`scripts/shared-chrome.test.mjs`](../../scripts/shared-chrome.test.mjs) answering CDP on `:9222` and [`scripts/bridge-attach.test.ts`](../../scripts/bridge-attach.test.ts) binding the live tab, so the shared-Chrome-plus-bridge arrangement is reachable and not just the Docker harness. **iOS is not**, for two independent reasons that a session cannot work around: the BrowserStack credentials are absent from it, and its network policy blocks `api.browserstack.com`, `hub-cloud.browserstack.com` and `emthought.cc` outright. So the honest shape is three environments rather than two — Copilot cloud with browser and iOS, Claude cloud with browser but no iOS, a laptop with whatever the developer starts.

Two properties of that runner do not apply to either of the others, and [`end-session`](skills.md#end-session) Step 8 is where they turn into rules. The runner is **reclaimed once the session goes idle**, and reopening the session provisions a fresh one. That makes idling the cheap state and waking the expensive one, which inverts the usual instinct to keep a session running: there is no compute charge for the runner, but every wake re-reads the whole conversation. A session whose work is finished should be allowed to end.
