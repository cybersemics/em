...
`write-issue` needed nothing — it shells out to `gh` and names no provisioned resource, and writing an issue someone else can reproduce is the same job in either environment.
