...
- **[`copilot-conflicts.yml`](../../.github/workflows/copilot-conflicts.yml)** scans Copilot-authored pull requests for merge conflicts and starts a follow-up Copilot session on the existing branch after a bounded delay. It reacts to pushes to `main` and relevant pull-request activity, persists its six-attempt lifetime retry state in one marked PR comment, and skips drafts along with any pull request labeled `skip-auto-resolve-conflicts` — a draft being a pull request `pr-ready.yml` has not released yet, so its own session may still be pushing to it. A dispatch naming a pull request (`-f pr=<number>`) overrides the wait and the cap, as `dependabot-fix.yml`'s does, but not those two exclusions; the comment it leaves shares its shape with that workflow's, down to the closing line naming the attempt and the run that started it. Covered in [Copilot pull-request conflicts](../testing.md#copilot-pull-request-conflicts).
