...
- **[`copilot-conflicts.yml`](../../.github/workflows/copilot-conflicts.yml)** scans Copilot-authored pull requests for merge conflicts and starts a follow-up Copilot session on the existing branch after a bounded delay. It reacts to pushes to `main` and relevant pull-request activity, persists its six-attempt lifetime retry state in one marked PR comment, and skips any pull request labeled `skip-auto-resolve-conflicts`. Covered in [Copilot pull-request conflicts](../testing.md#copilot-pull-request-conflicts).
