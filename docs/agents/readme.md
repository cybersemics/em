...
- **[`copilot-conflicts.yml`](../../.github/workflows/copilot-conflicts.yml)** scans Copilot-authored pull requests for merge conflicts and starts a follow-up Copilot session on the existing branch after a bounded delay. It reacts to pushes to `main` and relevant pull-request activity, persists its six-attempt lifetime retry state in one marked PR comment, and skips any pull request labeled `skip-auto-resolve-conflicts`. A dispatch naming a pull request (`-f pr=<number>`) overrides both the wait and the cap, as `dependabot-fix.yml`'s does; the comment it leaves shares its shape with that workflow's, down to the closing line naming the attempt and the run that started it. Covered in [Copilot pull-request conflicts](../testing.md#copilot-pull-request-conflicts).
