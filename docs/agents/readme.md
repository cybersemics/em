...
- **[`copilot-conflicts.yml`](../../.github/workflows/copilot-conflicts.yml)** scans Copilot-authored pull requests for merge conflicts and starts a follow-up Copilot session on the existing branch after a bounded delay. It reacts to pushes to `main` and relevant pull-request activity, and persists its six-attempt lifetime retry state in one marked PR comment. Covered in [Copilot pull-request conflicts](../testing.md#copilot-pull-request-conflicts).
