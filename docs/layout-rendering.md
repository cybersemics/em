...
So the key is every context-view boundary the path crosses plus the row's own step. A step that crosses a context view is a [*context step*](data-model.md#contextstep), which records the Lexeme context it lands on, so both halves distinguish occurrences that the ids alone could not: the same thought visible in normal view and in a context view, and two thoughts of the same Lexeme in one context (`a/m~/cat` and `a/m~/cats`) which both display as the same context.

Hashing the whole `Path` would also be unique, but it would change whenever an ancestor moves, remounting the node and breaking the move animation. The `sizes` map and TreeThought entries are keyed by `crossContextualKey`; the `data-path` attribute that drop animations select on is `hashPath(path)`, which the same tagging makes unique per row.
...
- **Context-view pivot.** When a thought has its context view active, the recursion pivots from "render this thought's children" to "render the *contexts* in which this thought appears" via [`getContextsSortedAndRanked`](../src/selectors/getContextsSortedAndRanked.ts). Each such child is appended with [`appendContextStep`](../src/util/appendToPath.ts), tagging the step with the Lexeme context so the boundary is recorded in the `Path` rather than re-derived downstream. `TreeThought.simplePath` is the *displayed* thought ([`parentContextPath`](../src/selectors/parentContextPath.ts), i.e. the parent context), which is what `<Editable>` edits and what metaprogramming attributes are read from; the row's children come from the Lexeme context. A special early-return: if the context view has only one context, the `NoOtherContexts` placeholder is rendered instead.
