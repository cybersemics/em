...
So the key is every context-view boundary the path crosses plus the row's own step. A step that crosses a context view is a [*context step*](data-model.md#contextstep), which records the Lexeme instance it lands on, so both halves distinguish occurrences that the ids alone could not: the same thought visible in normal view and in a context view, and two thoughts of the same Lexeme in one context (`a/m~/cat` and `a/m~/cats`) which both display as the same context.
...
- **Context-view pivot.** When a thought has its context view active, the recursion pivots from "render this thought's children" to "render the *contexts* in which this thought appears" via [`getContextsSortedAndRanked`](../src/selectors/getContextsSortedAndRanked.ts). Each such child is appended with [`appendContextStep`](../src/util/appendToPath.ts), tagging the step with the Lexeme instance so the boundary is recorded in the `Path` rather than re-derived downstream. `TreeThought.simplePath` is the *displayed* thought ([`contextThoughtPath`](../src/selectors/contextThoughtPath.ts), i.e. the context), which is what `<Editable>` edits and what metaprogramming attributes are read from; the row's children come from the instance. A special early-return: if the context view has only one context, the `NoOtherContexts` placeholder is rendered instead.
