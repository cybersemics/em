...
- **Root selection.** The traversal starts from `state.rootContext`: `HOME_PATH` normally, or `EM_PATH` when the EM context is the outline root (toggled by the **Edit Settings** command via [`toggleEmContext`](../src/actions/toggleEmContext.ts)). EM-rooted paths include `EM_TOKEN` explicitly as their first element, so their visual depth is one less than their length — `LayoutTree` subtracts the root token where it compares `cursor.length` against depth constants.
- **Visibility gating.** A subtree is skipped entirely unless `state.expanded[hashPath(path)]` is set, so collapsed branches don't appear in `treeThoughts` at all. The root itself is exempt (`isRoot`, or `isEM` for the EM root). (The expansion model itself lives in [`expandThoughts`](../src/selectors/expandThoughts.ts).)
