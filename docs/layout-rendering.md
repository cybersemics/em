...
`LayoutTree` now clamps `window.scrollY` to the autofocus-visible thought band instead of translating the tree:
...
3. The clamp bounds are computed with half a viewport of allowance above and below the band:
   - `minScrollY = max(0, layoutTreeTop + visibleTop - viewportHeight / 2)`
   - `maxScrollY = max(minScrollY, layoutTreeTop + visibleBottom - viewportHeight / 2)`
...
Net effect: users can still scroll to any displayed thought and about half a viewport beyond it, and touch users get an iOS-style elastic feel at the interior bounds while still settling inside the visible-thought band.
