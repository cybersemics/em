...
3. The hook returns the offset `autocrop = -spaceAboveExtended + viewportHeight`, applied as `transform: translateY(...)` on the outer container.
4. When `autocrop` changes, `window.scrollTo({ top: window.scrollY + delta })` keeps visible thoughts positionally stable.

The counter-scroll tracks the delta of `autocrop`, not of `spaceAboveExtended`. The two are equivalent when only `spaceAbove` changes, but a viewport-only change moves `spaceAboveExtended` without moving the offset — so compensating for it would scroll the page when nothing underneath it had shifted, pushing the cursor off screen ([issue #5670](https://github.com/cybersemics/em/issues/5670)). Every platform is configured to hold the viewport at full height while the virtual keyboard is open — `interactive-widget=overlays-content` on the web, and on Android the combination described in [`androidCapacitorHandler`](../src/device/virtual-keyboard/handlers/androidCapacitorHandler.ts) — so this case should not arise, but the offset delta is the correct quantity to compensate either way.
...
[`scrollCursorIntoView`](../src/device/scrollCursorIntoView.ts) scrolls the minimum amount needed to clear the toolbar above and the navbar (or virtual keyboard) below. [`useScrollCursorIntoView`](../src/hooks/useScrollCursorIntoView.ts) calls it three ways: directly when the cursor's `y` or `height` changes, when the virtual keyboard opens, and through [`scheduleScrollCursorIntoView`](../src/device/scheduleScrollCursorIntoView.ts) when the editing value changes — the cursor's rank can move as it is edited, e.g. toggling bold in a long, sorted context.

On touch platforms the keyboard overlays content rather than resizing the viewport — the Capacitor apps by configuration, mobile Chrome because [`androidWebHandler`](../src/device/virtual-keyboard/handlers/androidWebHandler.ts) opts into `navigator.virtualKeyboard.overlaysContent` — so `visualViewport.height` cannot say how much of the screen it covers. The height comes instead from [`virtualKeyboardStore`](../src/stores/virtualKeyboardStore.ts) and [`viewportStore`](../src/stores/viewport.ts), which the platform keyboard handlers populate from native events. iOS Safari is the exception: its visual viewport really does shrink, so `visualViewport.height` already accounts for the keyboard there. The keyboard covers the screen without moving the cursor, which is why its opening is a call path of its own — the `y`/`height` effect cannot see it. The bottom boundary also holds back one line of cursor height while the keyboard is up: autocrop re-runs after the scroll and shifts content down, so a cursor judged against the raw boundary reads as visible while actually sitting behind the keyboard, and a thought created at the bottom of the list is never scrolled to.
