...
The counter-scroll tracks the delta of `autocrop`, not of `spaceAboveExtended`. The two are equivalent when only `spaceAbove` changes, but a viewport-only change moves `spaceAboveExtended` without moving the offset — so compensating for it would scroll the page when nothing underneath it had shifted. This is what the Android WebView does when the virtual keyboard opens ([issue #5670](https://github.com/cybersemics/em/issues/5670)); mobile Chrome is unaffected because `interactive-widget=overlays-content` keeps the visual viewport at full height.
