...
Document commands receive an explicit `ThoughtspaceTransaction` and execute outside Redux's reducer. [`undoRedoEnhancer`](../src/redux-enhancers/undoRedoEnhancer.ts) wraps a complete dispatched command in one memory transaction; nested commands read canonical state after each update. It then records history and publishes one immutable state through Redux, even while SQLite persistence is pending. UI-only reducers remain pure. See [persistence.md](persistence.md#command-coordination-and-redux-publication).
...
[`undoRedoEnhancer`](../src/redux-enhancers/undoRedoEnhancer.ts) records the operation IDs authored by each command. Undo asks TreeCRDT to derive compensating operations from canonical history; redo reverts those new operations. Both run synchronously in the same memory transaction before publication, with asynchronous SQLite acknowledgement. The existing multicursor and edit grouping below determines history steps, not the transaction boundary. Initialization and incoming engine snapshots use non-undoable `replaceThoughts` publication.

Each entry in `state.undoPatches` holds a JSON diff (`ops`), patch-level `metadata` and `documentOperationIds`. JSON operations restore UI state and retain document changes for history reports; they never write the canonical document. Every patch records underlying `actionTypes` in first-occurrence order and whether all its actions are navigational. Command patches additionally record an invocation id, command id, authored label, optional input method and optional keyboard-shortcut index. Programmatic execution leaves the input method undefined; direct edits retain action metadata without command attribution. Action-based behavior reads `actionTypes`, while labels and reproduction steps read the command fields. Undo preserves this metadata while moving an entry to `redoPatches` with a fresh opposite diff and compensating operation IDs; redo moves it back. UI-only entries that have become no-ops are dropped. `redoPatches[0]` is the oldest undone entry, i.e. the newest point in the history.

This prototype uses **force-revert**, not selective collaborative undo: an intervening remote payload edit can be overwritten. Redo restores what the undo replaced, including that remote value. Missing operations or stale structural destinations/anchors reject the whole command. Inversion currently replays retained history synchronously; large histories and deep deleted subtrees can block the editor. Undo stacks themselves remain session-local.
...
While a request is pending, `generating` and `displayValue` supply the UI's progress text without changing the canonical thought value. Ellipses are not persisted, exported or added to lexeme memberships. A response cannot overwrite a target that was edited, deleted or canceled while waiting; completion clears the overlay and commits the generated value through the normal document boundary. Generate Emoji uses the same separation between progress text and document text.
