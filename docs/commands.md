...
`handleGestureSegment` is called incrementally as the user swipes; it triggers a haptic for each new segment and, after `COMMAND_PALETTE_TIMEOUT`, opens the gesture menu so the user can see all commands reachable from the current sequence. The menu is not opened while the pinned command tooltip is open (see [learning.md](learning.md)), since the user is then practicing the gesture the tooltip shows.
...
- [`CommandUniversePageTransitions`](../src/components/CommandUniverse/CommandUniversePageTransitions.tsx) gives each retained surface a declarative target and lets Motion move it there, with `useReducedMotion` skipping the zoom when requested. Because the target is declarative, navigating while a zoom is running simply re-targets: Motion continues from wherever the surface got to rather than restarting. Completion comes from Motion rather than a parallel timer, except when navigation requests `transition: 'none'`, which renders settled and finishes before paint. The default is `transition: 'zoom'`; reduced-motion preferences are applied separately. [`commandUniverseFinishTransition`](../src/actions/commandUniverseFinishTransition.ts) ignores a late completion from an abandoned animation. Inactive and transitioning pages are inert, but the header stays outside the page surfaces and so remains usable throughout. Opening a detail from the grid restores its heading or last focused element without scrolling. Back and Forward also restore focus on arrival.

Each history entry records the zoom direction and source point, relative to the page, of the navigation that created it. The point is measured before the new page mounts, so starting the zoom does not force layout over the populated grid. Back reverses that entry's zoom and Forward replays it, so a new entry can zoom either inward or outward. Navigation is never blocked by a running animation: Back and Forward stay enabled throughout, so a mistaken tap can be undone without waiting the zoom out. Reachable history entries remain mounted with their independent scroll positions. Starting a new branch discards its abandoned redo pages. Closing unmounts the page surfaces after the dialog fade, and reopening starts a fresh session even during an unfinished close fade. This history is independent of browser history and the editor's undo/redo.

Motion defaults live in [`commandUniverseMotion`](../src/components/CommandUniverse/commandUniverseMotion.ts), using the shared duration utility. The transition view reads them directly, and an enclosing Motion `MotionConfig` overrides them where one is present, which is how tests run the animation at zero duration. Page zooms use complete `transform: scale(...)` targets so Motion can run them through native Web Animations alongside opacity and blur; Motion's independent `scale` value depends on the JavaScript frame loop and can stall when navigation finishes.
...
After the invocation, the runner emits one `commandSucceeded` action for the command the user invoked, even when that command resolves to another command through Repeat. It waits for promises returned by `exec` or `execMulticursor`, and does not report success when execution is blocked, throws, rejects, sets a new app error, or returns `false` to mark cancellation. The learning reducer consumes this generic result to award at most one practice rep per shortcut or gesture invocation; see [learning.md](learning.md).
