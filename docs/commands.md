...
Preventing the native operation costs the WebKit route its redo gesture. WebKit puts a step on its redo stack only when it performs an undo itself, so once `beforeInput` prevents one there is nothing for it to redo and the native redo gesture is never delivered — iOS confirms the gesture with its own "Redo" overlay while nothing is restored ([issue #5575](https://github.com/cybersemics/em/issues/5575)). `registerNativeRedoStep` puts a step back: it inserts an empty marker into the editable and immediately undoes it natively, moving that step onto the redo stack. The DOM effect is immaterial — the insert is undone before the function returns, and `suppressChange` in the [`editableSync`](../src/stores/editableSync.ts) store hides both mutations from the Editable change handler, so no edit is recorded and the editable is not re-rendered. `handleNativeHistory` calls it after every native gesture em handles, whichever route the gesture arrived by, because a step is not only missing after the first prevented undo but goes *stale* after every one: em's undo re-renders the editable the step points at, and WebKit silently discards a step whose DOM the re-render replaced, dispatching nothing when the next gesture arrives. For the same reason the call is deferred past the re-render, by `NATIVE_HISTORY_REGISTER_DELAY` on the touch route — which runs at `touchend`, long before it — and by a bare task on the `beforeinput` route, which already arrives late enough. Registration needs a focused editing host, and em's editing hosts are the thoughts themselves, so undoing the creation of the only thought would leave the route with nowhere to register and nowhere to be delivered. [`device/nativeHistoryAnchor.ts`](../src/device/nativeHistoryAnchor.ts) supplies one that always exists: a 1px transparent `contenteditable` appended to the body, focused only when the insert finds no editable selection. `inputmode=none` keeps the software keyboard shut when it takes focus and `pointer-events: none` keeps it out of reach of taps; it carries no `data-editable`, so `selection.isThought` is false for it and the [`clearSelection`](../src/redux-middleware/clearSelection.ts) middleware leaves its focus alone.

That step is what a **shake** depends on, since a shake reaches em through `beforeInput` alone. The **three-finger swipe** does not depend on it, because iOS delivers the swipe to the page as ordinary touch events before it reports any `beforeinput` — so [`device/nativeHistory.ts`](../src/device/nativeHistory.ts) recognizes the gesture itself in Mobile Safari, from three touches travelling further horizontally than vertically, and calls `handleNativeHistory` directly. That reads no browser state, so it holds whether or not a step was successfully registered. A swipe handled that way leaves no `beforeinput` to arrive at all — with no editing host focused, WebKit dispatches nothing — which is why the step is refreshed from `handleNativeHistory` rather than from `beforeInput`: otherwise a shake taken after a swipe would find only a stale step and do nothing. When both signals do arrive for one swipe, the touch route stamps [`nativeHistoryGestureStore`](../src/stores/nativeHistoryGestureStore.ts) and `beforeInput` skips any `historyUndo` / `historyRedo` within `NATIVE_HISTORY_GESTURE_TIMEOUT` of it — still preventing the default, so WebKit cannot mutate the contenteditable, but not applying the gesture a second time.
