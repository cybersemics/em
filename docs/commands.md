...
| Option | Meaning |
|---|---|
| `execMulticursor(cursors, dispatch, getState, commandContext)` | Custom replacement for the per-cursor loop. Async implementations scope their completed edits with `commandContext.withCommandMetadata`. |
...
The whole of `executeCommandWithMulticursor` is synchronous, including that bracket, so an **asynchronous** command gets no help from it: the bracket is opened and closed around the call, and anything dispatched after the first `await` lands outside it. This is intentional—the global command transaction must not remain open while a request is in flight, because unrelated user input would be absorbed into the command's undo entry. Async commands use `commandContext.withCommandMetadata` around only the synchronous dispatches that apply their completed work. [`generateThought`](../src/commands/generateThought.ts), [`generateEmoji`](../src/commands/generateEmoji.ts), and [`defineTerm`](../src/commands/defineTerm.ts) also use a separate multicursor bracket to collapse all selected results into one undo step.
