...
Splits multiple sentences in a single thought into separate thoughts. A parenthetical at the end of the thought is split off into a child before anything else, e.g. `This is a thought (and a subthought)`; otherwise sentence punctuation (`.;!?`) takes priority over every other delimiter. When the sentences become siblings of a thought that already has siblings, they are placed in a new empty category, which keeps them distinct from those siblings, and the cursor is placed on the category so that it can be named right away. A thought with no siblings is split in place, since there is nothing to distinguish the sentences from, and so is a split that creates a child rather than siblings.
