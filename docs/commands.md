...
The command transaction brackets synchronous execution and closes as soon as a command returns, even when it returns a promise. [`withCommandDispatch`](../src/util/withCommandDispatch.ts) supplies a dispatch bound to the invocation: action arrays and nested thunks retain that dispatch, including after `await`, and completed dispatches automatically open a synchronous boundary. Command authors use the supplied `dispatch` normally; they do not opt into metadata handling. A command waiting on a request therefore leaves no global command transaction open that could absorb unrelated user input. A resumed invocation can extend its latest patch only while that patch remains contiguous in history; an intervening edit or undo/redo starts a new group. Invocation identity never merges across another history entry. [`generateThought`](../src/commands/generateThought.ts), [`generateEmoji`](../src/commands/generateEmoji.ts), and [`defineTerm`](../src/commands/defineTerm.ts) also use a separate multicursor bracket to collapse all selected results into one undo step.
...
Each undo entry on `state.undoPatches` is a `Patch` ([`undoRedoEnhancer`](../src/redux-enhancers/undoRedoEnhancer.ts)) with two parts: `ops`, the fast-json-patch diff that reverts the change, and patch-level `metadata`. Every patch records the underlying `actionTypes` in first-occurrence order and whether all its actions are navigational. Command patches additionally record an invocation id, command id, label, activation surface (`keyboard`, `gesture`, `toolbar`, or a command universe), and optional keyboard-shortcut index. Direct edits retain action metadata without command attribution. Action-based behavior reads `actionTypes`; command labels and reproduction steps read the command fields. The metadata is copied unchanged when Undo or Redo recomputes the operations and moves the patch between stacks, so even an empty recomputed diff retains its identity safely. `redoPatches[0]` is the oldest undone entry, i.e. the newest point in the history.

Command boundaries are plain `beginCommand` and `endCommand` actions handled by the undo enhancer. They do not change document state or create patches, but appear in the action stream for debugging and replay. The store routes these boundaries past document-effect middleware, preserving live throttled input, selection, and background scheduling; logging still receives them. A bound dispatch retains immutable invocation provenance, so nested executors inherit the enclosing command even when they resume asynchronously. Nested synchronous calls share the outer command's metadata and merge bookkeeping; ending an inner scope restores the enclosing scope. Boundaries close in `finally` even when execution throws. There is no store registry or out-of-band metadata setter. Repeat checks the invocation id on the newest non-navigation patch so a later no-op execution of the same command cannot replace the last successful command.

An **undo step** may contain more than one patch. Command transactions, contiguous text edits, and explicitly bracketed multicursor batches each produce a patch with one source of metadata, while the established directional grouping remains unchanged: Undo groups trailing navigation with the destructive patch before it, and a newly created thought with the edit that gives it a value; Redo applies the corresponding grouping in its forward direction. [`getUndoStepCount`](../src/util/getUndoStepCount.ts) owns the directional boundary rules used by the reducers and [`undoSteps`](../src/selectors/undoSteps.ts). The slider retains its structural grouping of formatting with a newly created thought, while keyboard Undo uses the live diff to keep formatting separate. The `undo` and `redo` actions accept an optional patch `count` so the slider can move across whole grouped steps.
