...
Each entry in `state.undoPatches` holds a JSON diff, action metadata and document operation IDs. JSON operations restore UI state and retain document changes for history reports; they never write the canonical document. Labels and typing/navigation grouping remain EM responsibilities. Undo moves an entry to `redoPatches` with a fresh opposite diff and the compensating operation IDs, and redo moves it back. UI-only entries that have become no-ops are dropped.
