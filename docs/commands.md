...
[`undoRedoEnhancer`](../src/redux-enhancers/undoRedoEnhancer.ts) evaluates commands before computing history diffs, so document patches describe canonical memory snapshots, including sibling-index ranks. Applying an undo or redo patch restores document updates and placements through the same memory transaction before publication. Undo therefore authors new CRDT operations instead of replacing Redux's document projection independently. SQLite acknowledges those operations asynchronously. The existing multicursor and edit grouping below determines history steps, not the memory transaction boundary. Initialization and incoming engine snapshots use non-undoable `replaceThoughts` publication rather than planning document writes.
