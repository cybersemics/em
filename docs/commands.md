...
The first branch in `beforeInput` intercepts native undo/redo (`inputType` `historyUndo` / `historyRedo`), which is what the iOS shake-to-undo and three-finger-swipe gestures dispatch. Left unhandled, WebKit mutates the contenteditable DOM directly, bypassing em's undo and leaving stale formatting markup ([issue #3954](https://github.com/cybersemics/em/issues/3954)). The branch calls `preventDefault` and dispatches em's `undo` / `redo` instead, so the thought is restored from Redux and re-rendered. Note that WebKit only dispatches this event when its own native undo stack has a step to undo — see `registerNativeUndoStep` in `formatSelection` for how a step is registered, and the caveats documented there ([issue #4637](https://github.com/cybersemics/em/issues/4637)).

There's a special case in `beforeInput` for `newThought` and `indent` to handle iOS auto-capitalization: the Enter / space character is prevented in the `beforeinput` event rather than `keydown` ([issue #3707](https://github.com/cybersemics/em/issues/3707)). A third branch in `beforeInput` handles Android: soft keyboards report the space keydown as `keyCode 229` (`'Unidentified'`), so it never matches the `indent` command in `keyDown` and `keyCommandId` is never set — the branch catches the `beforeinput` `insertText` of a single space over an empty thought and dispatches `indent` directly ([issue #4178](https://github.com/cybersemics/em/issues/4178)).
