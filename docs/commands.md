...
| Option | Meaning |
|---|---|
| `execMulticursor(cursors, dispatch, getState)` | Custom replacement for the per-cursor loop. May return a Promise; selection restoration and the command transaction wait for it to settle. |
...
An asynchronous `execMulticursor` returns its Promise to the executor. The command transaction and `isMulticursorExecuting` remain active until it settles, and selection restoration runs afterward. [`generateThought`](../src/commands/generateThought.ts) uses this path to generate all selected thoughts concurrently while recording the resulting edits as one command patch.
...
Each undo entry on `state.undoPatches` is a `Patch` ([`undoRedoEnhancer`](../src/redux-enhancers/undoRedoEnhancer.ts)) with two parts: `ops`, the fast-json-patch diff that reverts the change, and patch-level `metadata`. Command metadata records the command id, label, activation surface (`keyboard`, `gesture`, `toolbar`, or a command universe), optional keyboard-shortcut index, and whether the command is navigational. Direct actions use action metadata instead. The metadata is copied unchanged when Undo or Redo recomputes the operations and moves the patch between stacks, so even an empty recomputed diff retains its identity safely. `redoPatches[0]` is the oldest undone entry, i.e. the newest point in the history.

An **undo step** is exactly one patch. A command transaction collects all undoable actions produced by one command—including chained gestures, multicursor work, and asynchronous custom multicursor execution—into that patch. Direct contiguous text edits and explicitly bracketed non-command multicursor batches retain their existing coalescing. [`undoHistory`](../src/selectors/undoHistory.ts) presents the two stacks newest first and reports the current patch position. The `undo` and `redo` actions normally move one patch; their optional `count` lets the slider move directly across several patches.

The **undo slider** ([`UndoSlider`](../src/components/UndoSlider.tsx), toggled by its toolbar button or by a long press on Undo or Redo) is an rc-slider range over patch history, capped at ten, with the present at the right. Its two handles start together at the present with the *start* handle on top. Dragging the start handle back reveals the *end* handle, which always stays at least one patch after the start (except at the present, where the two coincide); the start handle stops one patch before the end handle rather than pushing it. Dragging or tapping either handle moves the thoughtspace to the point in time under it, so after dragging the end handle the user sees the end point while the start handle is preserved, and a tap on the start handle returns to the start point. The command label or direct-action label that produced each handle's point in time is rendered under the handle. The handles live in the state of `UndoSlider` rather than of the slider itself, which is unmounted while the slider is closed, so that closing and reopening it leaves them where the user put them. They reset to the current state whenever the total number of patches changes, i.e. when a new action discards the history ahead of the current state and they no longer refer to the same points. Opening the slider flushes any pending throttled edit through `commandEmitter`, and so does each move: editing dispatches `editThought` on a throttle, and the toolbar does not flush it the way the keyboard and gesture paths do, so an edit typed just before the slider was opened would be missing from the history and lost as soon as the slider moved the thoughtspace.

The copy button to the right of the slider copies a **bug report** for the patches between the handles ([`stepsToReproduce`](../src/selectors/stepsToReproduce.ts)): under *Steps to Reproduce*, the whole thoughtspace at the start point, exported as plain text with meta attributes inside a markdown code block (omitted when it is empty, which exports as the root's placeholder value rather than as nothing), followed by one numbered step per patch up to the end point; under *Current Behavior*, the thoughtspace at the end point; and an empty *Expected Behavior* heading to fill in. The selector reconstructs the state before and after every patch by applying history from the current state. Command patches describe how the command was invoked—such as ``Press `Shift + Backspace`.`` or ``Tap the Delete button.``—using the recorded activation surface and keyboard-shortcut index, then infer the affected thought or structural effect from the operations and surrounding states. Direct-action patches retain the existing action-specific descriptions. Cursor and selection setup is inserted when it differs from the state left by the preceding patch.
