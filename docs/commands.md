...
Top-level invocations dispatch `commandSucceeded` after returned asynchronous work settles. The report preserves command identity and whether the input was genuinely user-initiated; cancellation, rejection, and a new app error suppress it. AI disclosure returns the pending request. This execution-boundary contract does not verify every command-specific effect or internally scheduled work.
