...
While the request is in flight, each selected thought is marked `generating` without changing its stored value. An empty thought shows a "Generating Thought" placeholder; a thought that already has text keeps that text. Both get a shimmer highlight until the request settles.
...
While the request is in flight, each selected thought is marked `generating` without changing its stored value. An empty thought shows a "Generating Emoji" placeholder; a thought that already has text keeps that text. Both get the same shimmer highlight as Generate Thought until the request settles.
...
While the request is in flight, each reorganizable thought is marked `generating` without changing its stored value. An empty thought shows a "Reorganizing Thought" placeholder; a thought that already has text keeps that text. Both get the same shimmer highlight as Generate Thought until the request settles.
