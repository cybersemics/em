...
After the invocation, the runner emits one `commandSucceeded` action for the command the user invoked, even when that command resolves to another command through Repeat. It waits for promises returned by `exec` or `execMulticursor`, and does not report success when execution is blocked, throws, rejects, or sets a new app error. The learning reducer consumes this generic result to award at most one practice rep per shortcut or gesture invocation; see [learning.md](learning.md).
