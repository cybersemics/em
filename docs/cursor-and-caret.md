...
A non-Redux ministore tracking which formatting applies at the caret — bold, italic, underline, strikethrough, code, and the text and background colors — which is what highlights a formatting button in the toolbar and marks a swatch in the [`ColorPicker`](../src/components/ColorPicker.tsx) as selected. A selected swatch means the next tap on it removes the color rather than applying it, so a stale value silently turns a tap into a no-op. A collapsed caret formats the whole thought — that is how a color is applied without selecting anything — but a caret sitting inside a colored chunk is already reported as selected, so removing the color that way would take every other chunk of the thought that shares it with it. [`formatSelection`](../src/actions/formatSelection.ts) therefore passes the caret offset along with the widened range, and [`formatSelectionHtml`](../src/util/formatSelectionHtml.ts) narrows a color-clearing command back to the colored element that surrounds the caret. Repeated taps still clear the thought a chunk at a time.
