...
Two mechanisms keep the caret where it belongs, both scoped to `isTouch && isSafari()`:

- **Only the cursor thought is an editing host.** While `state.isKeyboardOpen` is true, `Editable` passes `disabled` to `ContentEditable` for every thought that is neither the cursor nor `transient`, so the trackpad has nothing else to focus. This is deliberately *not* folded into `Editable`'s own `disabled` selector, which is a document/drag-level concept that `handleTapBehavior` also reads — routing it through there would take the preventDefault branch on every tap and place the caret at offset 0 instead of where the user tapped. Tapping a non-cursor thought still works, but needs one extra step: `useEditMode`'s mousedown handler derives the offset from the tap coordinates via [`getCaretOffset`](../src/device/getCaretOffset.ts), then promotes the element to an editing host *before* placing the caret, because the browser silently drops a caret placed in a non-editable element. The tap makes that thought the cursor, so React renders it editable on the next commit anyway; the imperative promotion only ensures the caret survives the intervening moment.
- **A stranded selection is pulled back.** Making the neighbours non-editable stops the *focus* from moving but not the *selection*, which the trackpad can still drop into non-editable text. A `selectionchange` listener in `useEditMode` restores the caret to the cursor thought's editable when the selection has left it. It runs only while that editable is still `document.activeElement`, which is what distinguishes this from the many flows that legitimately move the selection away after blurring it first (`selection.clear`, the Command Center, drag start), and only when no finger is on the page — neither a press in progress (`pressingRef`, a long press dragging a selection within the thought) nor one that ended within the ghost-mouse window (a tap, which may deliberately have moved the caret to a thought that is about to become the cursor). The drag only ever leaves past the left edge, so the caret is restored to offset 0 rather than to a remembered position, which cannot go stale against an edit that shortens the thought. The restore runs synchronously in the handler rather than being deferred, so the DOM is corrected as soon as the escape is known; a browser that declines the set fires no `selectionchange` at all, so it cannot spin.

**Known limitation: something caret-like is still *seen* travelling to the drag point and back.** Three results, all from a physical device (it does not reproduce in the Simulator on iOS 18.6, and the reporting device runs Safari 26.6):

- Adding `user-select: none` to the non-cursor thoughts stops the selection escaping at all — 56 consecutive `selectionchange` events all reported the cursor thought — and the travelling artifact continues regardless. So it does not track the browser selection.
- `caret-color: transparent` set on the document element hides the *real* caret and leaves the travelling one untouched.
- Restoring the selection synchronously rather than a frame later makes no difference.

The leading explanation is that iOS draws this in its own text-interaction layer, composited above the web content, where page CSS cannot reach it. That is an inference rather than a verified mechanism, and it is worth knowing what is still open before spending effort here: nobody has confirmed the artifact is a *caret* rather than a selection handle or the magnifier loupe, which would bring different properties into play (`-webkit-touch-callout`, `::selection`); and the `caret-color` result was applied by inheritance from the document element, so a more specific declaration on a descendant — the `hideCaret` keyframes in `panda.config.ts` set `caretColor` themselves — could have overridden it rather than the property being unreachable. Reading the computed `caret-color` on the element the artifact passes through would settle that one cheaply.
