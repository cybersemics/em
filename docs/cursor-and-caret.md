...
- A note follows the same pattern ([#4827](https://github.com/cybersemics/em/issues/4827)): the first tap on a note moves the cursor to the note's thought without opening the keyboard, and a tap on a note whose thought already has the cursor focuses the note and opens the keyboard. As with thoughts, a tap while the keyboard is already open enters the note directly.
...
The first tap suppresses edit mode by calling `preventDefault()` on its touchend (in `Editable`'s `handleTapBehavior`, and in `Note`'s `onTouchEnd` for a note), which normally stops the browser from synthesizing the tap's mousedown/focus/click and thereby keeps the keyboard closed. Browsers sometimes fire those synthesized events anyway — e.g. iOS Safari when the touchend is not cancelable because the tap landed during scroll momentum, or Mobile Chrome after drag cleanup has completed. `globals.suppressCursorAfterTouch` closes this hole: `handleTapBehavior` sets it when a touchend moves the cursor or toggles the multiselect without entering edit mode, `Note`'s `onTouchEnd` sets it when a touchend moves the cursor to the note's thought, and touch drag-end sets it before returning `state.longPress` to `Inactive`. While it is set, `Editable` drops tap, focus, and mousedown paths that could move the cursor or open the keyboard, and `Note`'s `onFocus` dismisses the focus instead of opening the keyboard. A capture-phase touchstart listener in [`initEvents`](../src/util/initEvents.ts) clears it, since a new touch proves that subsequent cursor events belong to a genuine new tap. Focus suppression is bypassed when `state.isKeyboardOpen` is already true so that commands which activate edit mode by side effect can still focus programmatically.
