...
[`src/device/scrollCursorIntoView.ts`](../src/device/scrollCursorIntoView.ts) keeps the cursor within a comfort zone: the area between the current top and bottom autoscroll edges. [`autoscrollBounds`](../src/device/autoscrollBounds.ts) owns the DOM and virtual-keyboard measurements that define those edges. Its bottom edge uses the keyboard's final height when known and otherwise preserves the browser's `visualViewport` fallback.
