...
It is called once per touch, from the capture-phase touchstart listener in [`initEvents`](../src/util/initEvents.ts), which latches the answer in [`globals.pressOnCaret`](../src/globals.ts). Latching there rather than in each reader ties the flag's lifetime to the touch instead of to whichever elements happen to mount a handler, and puts it ahead of both readers. It measures `changedTouches[0]`, the finger that just landed; `touches[0]` is the first finger still down, which a second finger arriving mid-edit would measure instead.

Three gates depend on the flag, and all three are needed because they guard independent subsystems ([issue #3763](https://github.com/cybersemics/em/issues/3763)). None of this is iOS-only — nothing in the path is platform-gated, so it applies to any touch platform, including the Android caret handle:
