...
It is called once per touch, from the capture-phase touchstart listener in [`initEvents`](../src/util/initEvents.ts), which latches the answer in [`pressOnCaret`](../src/stores/touchStore.ts). Latching there rather than in each reader ties the flag's lifetime to the touch instead of to whichever elements happen to mount a handler, and puts it ahead of both readers. It measures `changedTouches[0]`, the finger that just landed; `touches[0]` is the first finger still down, which a second finger arriving mid-edit would measure instead.

Three gates depend on the flag, and all three are needed because they guard independent subsystems ([issue #3763](https://github.com/cybersemics/em/issues/3763)). The latch is scoped to `isTouch && isSafari()`, so only iOS Safari and the Capacitor WKWebView are affected; Android's caret handle is drawn below the text line rather than on it, and its drag-and-drop and gestures behave as they did before:

- [`useLongPress`](../src/hooks/useLongPress.ts) does not mark a press that lands on the caret, so no `DragHold`.
- `canDrag` reads the flag, so react-dnd's own timer cannot start a drag behind `useLongPress`'s back. It reads the flag rather than `state.longPress` because react-dnd is not part of the long press state machine and can reach `DragInProgress` from `Inactive`, which the reducer explicitly permits.
- `shouldCancelGesture` in [`AppComponent`](../src/components/AppComponent.tsx) abandons a gesture that starts on the caret. Without it, suppressing `DragHold` would *un*-cancel gestures during caret repositioning: a gesture is abandoned at touchstart or when `state.longPress` leaves `Inactive`, and the latter is what used to fire. Reading the flag rather than a coordinate also covers `shouldCancelGesture`'s mid-gesture call, which carries no coordinates.
