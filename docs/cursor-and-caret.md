...
- **Only the cursor thought is an editing host.** While `state.isKeyboardOpen` is true, `Editable` passes `disabled` to `ContentEditable` for every thought that is neither the cursor nor `transient`, so the trackpad has nothing else to focus. This is deliberately *not* folded into `Editable`'s own `disabled` selector, which is a document/drag-level concept that `handleTapBehavior` also reads — routing it through there would take the preventDefault branch on every tap and place the caret at offset 0 instead of where the user tapped. Taps on other thoughts are unaffected because `useEditMode`'s mousedown handler derives the offset from the tap coordinates via [`getCaretOffset`](../src/device/getCaretOffset.ts), not from `contenteditable`.
