...
- The cursor is on a root child reached via the context view (`isRoot([parentContextId(state, cursor)])` — the thought the row *displays*, since a context step stores the Lexeme context rather than the context).
