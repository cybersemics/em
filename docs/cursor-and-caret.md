...
[Clear Thought](../src/commands/clearThought.ts) works on a multiselection: it clears every selected thought and keeps the multicursors alive so that subsequent typing is mirrored to all of them by `Editable`'s `onChangeHandler`, which dispatches an `editThought` per selected thought on every keystroke rather than through the edit throttle, so that they stay in sync character by character. Only one thought can hold the real browser caret, so the others render a faux caret to show that they are being edited too. The multicursors are keyed by path, so the handler is memoized on the thought's path among the rest of the state it reads: a thought moved by a command (Indent, for example) keeps its `Editable`, and a handler left memoized on the path the thought had before the move matches no multicursor and mirrors nothing ([issue #5288](https://github.com/cybersemics/em/issues/5288)).
