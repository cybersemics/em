...
- **Split helpers:** `split(el)` and `splitNode(root, range)` return the HTML before/after the caret with formatting tags re-balanced. `offsetFromClosestParent(root, offset)` resolves a plain-text offset to the node and node-relative offset a `Range` needs. [`splitHtmlAtTextOffset`](../src/util/splitHtmlAtTextOffset.ts) composes the two to split a value at a plain-text offset, which is how the Split Sentences and Extract Subthought commands cut a formatted thought.
...
Because those offsets index the plain text and the value is HTML, a command that acts on a range has to split the value rather than slice the string. `value.slice(selectionStart, selectionEnd)` lands in the middle of a tag as soon as the thought carries formatting, so the markup ends up in the extracted text and the remainder is left unbalanced ([issue #4103](https://github.com/cybersemics/em/issues/4103)). `splitHtmlAtTextOffset` does the cut; a range is taken by splitting at the end offset and then splitting that left half at the start offset, because a single `Range` spanning both boundaries drops the surrounding tags when both fall in the same text node ([issue #4229](https://github.com/cybersemics/em/issues/4229)). Re-joining the two remaining halves merges the tags that become adjacent, so that `<b>Lorem </b><b>dolor</b>` collapses back into one. [`extractSubthought`](../src/actions/extractSubthought.ts) does all of this, and keeps the string slice as a fast path for a value with no markup.
