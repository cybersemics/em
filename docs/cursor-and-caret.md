...
Closing the keyboard must leave no text selection behind. The Down Arrow and Android back button do not blur the editable, so the platform's [virtual keyboard handler](../src/device/virtual-keyboard/index.ts) reports the hide and [`dismissKeyboard`](../src/actions/dismissKeyboard.ts) clears the browser selection and exits edit mode ([#3958](https://github.com/cybersemics/em/issues/3958)). Tapping empty thoughtspace lets the browser blur the editable immediately on every platform, which exits edit mode on its own. A range must be collapsed before either path begins so native selection handles and the text context menu do not remain after the keyboard goes away ([#4833](https://github.com/cybersemics/em/issues/4833)).

Two mechanisms take that selection with the keyboard, and both **collapse** the range ([`selection.collapse`](../src/device/selection.ts)) rather than clearing it. Android draws the context menu for the range, so collapsing dismisses the menu without blurring. A range torn down while the keyboard is going away makes Android rebuild the menu around it, so the menu flashes back after it has already gone. The caret is dismissed by the tap's blur or by the native hide completion for the Down Arrow and back button.

Timing is therefore what each mechanism is chosen for. `Content` collapses a range on empty-space tap **down**, ahead of the browser's default blur. The Android Capacitor [keyboard handler](../src/device/virtual-keyboard/handlers/androidCapacitorHandler.ts) collapses a range at `willHide` and dismisses the caret at `didHide` for the native Down Arrow and Back paths. Empty-space taps blur immediately without adding a navigation offset.

On Android 11 and later, [`VirtualKeyboardTracker`](../android/app/src/main/java/com/emtheapp/em/VirtualKeyboardTracker.java) samples the current IME animation's interpolator, applies the system animator-duration scale, and derives its monotonic start time from a Choreographer frame timestamp and the first interior animation fraction. [`androidKeyboardAnimation`](../src/device/virtual-keyboard/androidKeyboardAnimation.ts) calibrates the native and JavaScript clocks with round-trip samples, then runs Web Animations on the main WebView's bottom-fixed controls. The independent `translate` property composes with popup swipe-dismiss `transform`. Later native samples can correct a changing shown-height endpoint without restarting the animation clock. No per-frame velocity prediction or keyboard-specific offset is applied. Controls stop at the resting navigation safe area. Raw height events provide positioning until clock calibration completes or when an older APK lacks the metadata API; stores update only at lifecycle milestones.

[`KeyboardWebView`](../android/app/src/main/java/com/emtheapp/em/KeyboardWebView.java) preserves the departing editor's `inputType` and `imeOptions` when Chromium returns a null input connection after blur. Some keyboards otherwise switch layouts in response to the disconnected editor's `TYPE_NULL` while the system is still animating the previous keyboard height. Only layout metadata is retained: the DOM editable blurs immediately, the connection remains null, and the next real editor supplies fresh metadata. The rule is independent of the selected keyboard and is restricted to Android 11+.

[`capacitor.config.ts`](../capacitor.config.ts) disables Android `SystemBars` inset handling: Capacitor otherwise pads the decor view by the full IME height and physically shrinks the WebView, so keyboard positioning is applied twice. [`MainActivity`](../android/app/src/main/java/com/emtheapp/em/MainActivity.java) strips the IME inset sent to the WebView container, preserving the full-height viewport throughout opening and closing. [`usePositionFixed`](../src/hooks/usePositionFixed.ts) registers bottom anchors with the animation controller when given an element ref; fullscreen popups opt out, and WebViews without independent CSS translation use raw CSS-height positioning. Android versions below 11 retain the existing Capacitor lifecycle behavior, and iOS and mobile-web handlers retain their platform positioning paths.
