...
Partial thought formatting is one of the cases where manual range preservation is unavoidable. [`formatSelection`](../src/actions/formatSelection.ts) computes the canonical HTML, writes it to the live editable, and calls `selection.setRange` synchronously before dispatching the edit without a forced render. [`ContentEditable`](../src/components/ContentEditable.tsx) skips an `innerHTML` assignment when the live HTML already matches the prop, so the React effect does not destroy the restored range. [`formatLetterCase`](../src/actions/formatLetterCase.ts) edits a sub-range too — the offsets it measures with `offsetRange` decide which text is letter-cased, not just which text is re-selected afterwards, and a collapsed caret or a full selection letter-cases the whole thought, as in `formatSelection`. It preserves the range the same way for the cursor thought: it writes the letter-cased value to the live editable, calls `selection.setRange`, and dispatches the edit without a forced render and without `setCursor`. Both omissions are load-bearing on touch. `force` bumps `editableNonce`, and `setCursor` recomputes `expanded` and resets `cursorCleared`; either re-render re-runs [`useEditMode`](../src/components/Editable/useEditMode.ts), which places the caret at `cursorOffset` on top of the range that was just restored — and because the intervening blur resets that offset to 0, the caret lands at the start of the thought with the keyboard still open. The offsets it measures with `offsetRange` decide which text is letter-cased, not just which text is re-selected afterwards. When there is no range to write synchronously — a multicursor whose cursor thought is not itself edited — it falls back to the forced render and a mutation observer that waits for the editable to re-render, since the transform can change the length of the text the range has to be mapped across. The text that observer waits for is derived from the value being dispatched, not from the editable's own text: [`Editable`](../src/components/Editable.tsx) trims the value on its way into Redux, so a thought with leading or trailing whitespace re-renders as text the editable never held, and an observer waiting on the editable's text would never fire.

Pickers that dispatch these edits must apply on **touchend**, as [`ColorPicker`](../src/components/ColorPicker.tsx) and [`LetterCasePicker`](../src/components/LetterCasePicker.tsx) both do. Applying on touchstart edits the editable while the finger is still down, and the remainder of the touch sequence then blurs it — closing the keyboard and dropping the selection the edit had just restored. Consuming the tap is a second, independent reason for the same rule (see [Mobile](#mobile)).
