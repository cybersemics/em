...
A non-Redux ministore tracking which formatting applies at the caret — bold, italic, underline, strikethrough, code, and the text and background colors — which is what highlights a formatting button in the toolbar and marks a swatch in the [`ColorPicker`](../src/components/ColorPicker.tsx) as selected. A selected swatch means the next tap on it removes the color rather than applying it, so a stale value silently turns a tap into a no-op. A collapsed caret formats the whole thought — that is how a color is applied without selecting anything — but a caret sitting inside a formatted chunk is already reported as selected, so removing the formatting that way would take every other chunk of the thought that shares it with it. [`formatSelection`](../src/actions/formatSelection.ts) therefore passes the caret offset along with the widened range, and [`formatSelectionHtml`](../src/util/formatSelectionHtml.ts) narrows the command back to the element that surrounds the caret: the colored element when the command clears a color, and the enclosing `b`/`i`/`u`/`strike`/`code` element when a tag command would otherwise format the whole thought. That first tap leaves the caret outside any formatted element, so the next tap applies the formatting to the whole thought and the one after removes it — the user can still format and unformat the whole thought, it just takes an extra tap.
