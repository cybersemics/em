...
### The native text selection menu

The menu the platform raises over a selected range (`Cut | Copy | Paste | Replace…`) is drawn by the browser, not by em. WebKit presents it only from a real touch on a live range — a programmatic selection shows handles but no menu until a touch lands on it, which is why [`showEditMenu`](../src/e2e/iOS/helpers/showEditMenu.ts) taps the selection — and it dismisses it on any mutation of the focused editable's content, whoever makes the mutation: an in-place `data` write, a `document.execCommand` editing command, and an `innerHTML` rewrite all dismiss it alike. A change of range alone does not; re-selecting the same range leaves the menu up.

Partial thought formatting is a mutation of exactly that kind. [`formatSelection`](../src/actions/formatSelection.ts) writes the new HTML to the live editable and restores the range immediately, and [`formatLetterCase`](../src/actions/formatLetterCase.ts) restores the range once the re-render lands. Both preserve the selection and leave the keyboard up, so applying Bold, a color, or a letter case to a word selected by long press leaves that word selected on WebKit — iOS Safari and both Capacitor apps — with the menu gone ([issue #5641](https://github.com/cybersemics/em/issues/5641)). Mobile Chrome is the exception, keeping its menu across the edit. Nothing in the web layer brings WebKit's back: extending or re-selecting the range, blurring and re-focusing the editable, and synthesizing a touch on the selection all leave it hidden, and there is no API to present it. The user's own next tap on the selection does, since the selection is still there. Keeping it up across a format would take a menu em presents itself, either natively per platform (`UIEditMenuInteraction`, Android's `ActionMode`) or in the app.
