...
- **A stranded selection is pulled back.** Making the neighbours non-editable stops the *focus* from moving but not the *selection*, which the trackpad can still drop into non-editable text. A `selectionchange` listener in `useEditMode` restores the caret to the cursor thought's editable when the selection has left it. It runs only while that editable is still `document.activeElement`, which is what distinguishes this from the many flows that legitimately move the selection away after blurring it first (`selection.clear`, the Command Center, drag start), and only when no finger is on the page — neither a press in progress (`pressingRef`, a long press dragging a selection within the thought) nor one that ended within the ghost-mouse window (a tap, which may deliberately have moved the caret to a thought that is about to become the cursor). The drag only ever leaves past the left edge, so the caret is restored to offset 0 rather than to a remembered position, which cannot go stale against an edit that shortens the thought. The restore runs synchronously in the handler rather than being deferred, so the DOM is corrected as soon as the escape is known; a per-frame attempt cap guards against a browser that refuses to move the selection back.

**Known limitation: the caret is still *seen* travelling to the drag point and back.** This is not the DOM caret and the page cannot suppress it — iOS draws its own caret in the native text-interaction layer, composited above the web content, decoupled from the browser selection. Two results pin this down: it persists when the selection provably never leaves the thought (adding `user-select: none` to the non-cursor thoughts stops every escape, and the flight continues), and `caret-color: transparent` hides the *real* caret while leaving the flying one untouched. Restoring synchronously rather than a frame later does not change it either. Do not spend another attempt on CSS here.
