...
- **Split helpers:** `split(el)` and `splitNode(root, range)` return the HTML before/after the caret with formatting tags re-balanced. Used by the Split Sentences command, by paste to cut out the text a selection replaces, by the Extract Subthought and Extract Category commands through [`splitFormattedValue`](../src/util/splitFormattedValue.ts), which they share to cut a formatted value into the part that stays and the part that is extracted, and by the Bump Thought Down command through [`splitEmojiPrefix`](../src/util/splitEmojiPrefix.ts), which cuts a formatted value after its leading emoji. Both go through [`splitHtmlAtTextOffset`](../src/util/splitHtmlAtTextOffset.ts), which maps a plain text offset onto the markup so that the cut never lands in the middle of a tag.

An edit that rewrites a thought's HTML addresses it by plain-text offset and resolves that offset through `collapsedRangeAtOffset`, never by indexing into the markup — a markup index cuts between two tag contexts and leaves a tag unclosed ([issue #5154](https://github.com/cybersemics/em/issues/5154)), and considers an HTML entity to have as many characters as its markup is long ([issue #5297](https://github.com/cybersemics/em/issues/5297)). Paste does both halves that way: [`splitHtmlAtTextOffset`](../src/util/splitHtmlAtTextOffset.ts) removes the range the selection replaces, and [`insertHtmlAtTextOffset`](../src/util/insertHtmlAtTextOffset.ts) puts the pasted HTML in. They share the offset-to-Range mapping but not the operation, and are not interchangeable — the split re-balances tags onto *both* halves, so composing the insertion as `left + pasted + right` would leave the pasted text outside the formatting both halves carry, splitting a bold run in two. The insertion instead lands inside the resolved text node, so it inherits the formatting there.
...
Notes own a separate contenteditable in [`Note`](../src/components/Note.tsx), so they place their caret directly instead of using `useEditMode`. Each note edit captures the post-edit character offset relative to the note root with `offsetFromNode()` and carries it on the note's edit action (`editThought` for a literal note or `editNotePath` for a path-based note). The undo enhancer uses the edit's plain-text length change to retain the corresponding pre-edit offset. Undo and redo force the note to render, then `Note` reads the requested `state.noteOffset` non-reactively, places the caret, and clears that one-shot request with a `setNoteFocus` that `undoRedoEnhancer` keeps out of the undo history. Placing the caret also focuses the note, which is why `onFocus` bails when state already has the caret on this note: that focus is the app's own, so there is no cursor to move, and the `setCursor` it would otherwise dispatch is an undoable navigation action — it would clear the offset the effect is in the middle of honoring, recompute `cursorOffset`, and discard the redo stack the moment an undo restores a note caret, leaving the note edit that was just undone with no redo step. Keeping the offset non-reactive prevents an ordinary click or text selection from being overwritten by a render.
...
- While a multiselect is active, a tap on a thought or on its bullet toggles that thought's selection instead of moving the cursor. Once more than one thought is selected, the cursor is moved out of the selection entirely so that the selected thoughts are all shown alike, and lands on the first selected thought when the selection ends. Deselecting the last selected thought ends the multiselect and restores the normal tap behavior. See [Multicursor](commands.md#multicursor). Only a touch that stays put counts as a tap here: `Editable` flags a touch that travels more than `TOUCH_SLOP` from where it began, so a gesture drawn on top of a thought or a scroll that starts on one does not add it to the multiselect ([#5269](https://github.com/cybersemics/em/issues/5269)). The flag is set during `touchmove` rather than measured at `touchend`, since a gesture that doubles back, such as →←, ends where it began. The bullet needs no such flag, as its toggle runs on a `click`, which the browser does not synthesize after a touch that moved.
...
Two mechanisms take that selection with the keyboard, and both **collapse** the range ([`selection.collapse`](../src/device/selection.ts)) rather than clearing it. Android draws the context menu for the range, so collapsing dismisses the menu — and because collapsing does not blur, the menu's dismissal does not overlap the keyboard's. That ordering is the whole point: a range torn down while the keyboard is going away makes Android rebuild the menu around it, so the menu flashes back after it has already gone. The caret left behind is dismissed by the blur that follows.

Timing is therefore what each mechanism is chosen for. `Content`'s empty-space tap handler collapses on tap **down**, ahead of the tap's own blur. That covers only the taps the page actually receives — in the Capacitor app the WebView's selection controller consumes the tap, and the Down Arrow is not a tap on the page at all — so the Android Capacitor [keyboard handler](../src/device/virtual-keyboard/handlers/androidCapacitorHandler.ts) collapses on `keyboardWillHide`, the last signal that arrives while the keyboard is still up. `keyboardDidHide` is too late: it does not fire until the keyboard has finished animating away, and by then the menu has already flashed. Mobile web has no will-hide signal at all — the Chromium `geometrychange` event reports a *hidden* keyboard (`boundingRect.height === 0`) only once the animation is over, and reports it just once, so the [Android web handler](../src/device/virtual-keyboard/handlers/androidWebHandler.ts) has to make the gap itself: it collapses the range on that event and defers the blur past the next paint. Collapsing dismisses the menu on its own, and letting that reach the compositor before the focus goes is what keeps Android from rebuilding the menu and flashing a second, read-only one back ([#5259](https://github.com/cybersemics/em/issues/5259)). `dismissKeyboard` remains the backstop that clears a live range whatever state edit mode is in, since edit mode has usually _already_ ended by the time the platform reports the hide — the tap's blur ends it, as does a long press that selects a word (see `Editable`'s `onFocus`), which would otherwise leave its edit-mode branch inapplicable and the selection stranded. A collapsed caret is left alone throughout: the blur dismisses it anyway, and clearing would blur the editable that a tap on a bullet is about to move the caret into.
...
The first tap suppresses edit mode by calling `preventDefault()` on its touchend (in `Editable`'s `handleTapBehavior`), which normally stops the browser from synthesizing the tap's mousedown/focus/click and thereby keeps the keyboard closed. Browsers sometimes fire those synthesized events anyway — e.g. iOS Safari when the touchend is not cancelable because the tap landed during scroll momentum, or Mobile Chrome after drag cleanup has completed. `suppressCursorAfterTouch` in the [`touch`](../src/stores/touch.ts) store closes this hole: `handleTapBehavior` sets it when a touchend moves the cursor or toggles the multiselect without entering edit mode, and touch drag-end sets it before returning `state.longPress` to `Inactive`. While it is set, `Editable` drops tap, focus, and mousedown paths that could move the cursor or open the keyboard. A capture-phase touchstart listener in [`initEvents`](../src/util/initEvents.ts) clears it, since a new touch proves that subsequent cursor events belong to a genuine new tap. Focus suppression is bypassed when `state.isKeyboardOpen` is already true so that commands which activate edit mode by side effect can still focus programmatically.
...
Every thought renders a `contenteditable`, so the document contains as many editing hosts as there are visible thoughts. That is fine for taps, which target one thought, but not for the iOS virtual keyboard's **trackpad** (long press the space bar): it moves the caret by hit-testing a point that follows the finger across the whole document, rather than by walking the focused editing host. Dragging past the edge of a thought therefore lands the caret in a neighbour, which focuses it — and `onFocus` makes whatever it focused the cursor. Because a cursor change re-runs [`expandThoughts`](../src/selectors/expandThoughts.ts) and the expansion follows the cursor, whole subtrees mount and unmount, so the thoughts move under the still-held finger and the next hit-test lands somewhere else again. The caret walks up the ancestor chain until it leaves the document entirely; the blur that follows sets `isKeyboardOpen` false while the keyboard is still visibly up, and typing stops working ([issue #3276](https://github.com/cybersemics/em/issues/3276)).

It is expansion rather than autofocus that supplies the feedback: [`VirtualThought`](../src/components/VirtualThought.tsx) preserves a hidden thought's height when it shims it out, precisely so that removing it does not move anything. Reproducing therefore needs a tree deep enough that moving the cursor up *collapses a subtree* — not merely one with a thought hidden by autofocus. A cursor on a leaf whose parent has other children is the reliable shape.

[`useCaretRestore`](../src/components/Editable/useCaretRestore.ts) keeps the caret where it belongs, scoped to `isTouch && isSafari()`. It registers a `selectionchange` listener that restores the caret to its editable when the selection has left it, and runs only while that editable is still `document.activeElement` — which is what distinguishes a trackpad drag, where the focus never moves, from the many flows that legitimately move the selection away after blurring it first (`selection.clear`, the Command Center, drag start). It also runs only when no finger is on the page: neither a press in progress (a long press dragging a selection within the editable) nor one that ended within the ghost-mouse window tracked by [`lastTouch`](../src/components/Editable/lastTouch.ts) (a tap, which may deliberately have moved the caret to a thought that is about to become the cursor). The restore runs synchronously in the handler rather than being deferred, so the DOM is corrected as soon as the escape is known; a browser that declines the set fires no `selectionchange` at all, so it cannot spin.

The caret is restored to one end of the text rather than to a remembered position, which cannot go stale against an edit that shortens the value. Which end depends on where the escape happens, and the two editables differ:

- **Thoughts** (`useEditMode` passes `contentRef`, enabled while the thought is the cursor and the keyboard is open) restore to **offset 0**. The drag only ever leaves past the left edge; dragging right keeps the selection clamped to the end of the thought and inside the editable.
- **Notes** (`Note` passes `noteRef`, enabled while it has note focus) restore to the **end**. A note is short enough that the trackpad's hit test lands outside it from the moment the space bar is pressed, so the caret escapes with no drag at all — and it escapes from the end, where the note abuts the parent thought.

Putting the caret back does not end the drag, and the caret that was just restored becomes the anchor the drag extends from: the next hit-test selects everything between it and the finger, leaving the whole thought highlighted. The hook therefore latches while a drag is underway — set on every restore, cleared by a `touchstart` on the editable and on unmount — and collapses any range that appears while the latch is set. The latch is what keeps the rule narrow. A range is not on its own evidence of anything wrong: Select All from the edit menu produces one, the native selection handles produce one, and Extract Subthought and Extract Category read the cursor thought's selected range as their input. None of those follow a caret escape, and the ones driven by a finger clear the latch before they start — which matters most for the handles, since they are native UI drawn outside the DOM and may never deliver a touch event to the editable at all.

Restoring the selection is enough on its own, which is not obvious: `selectionchange` fires before WebKit moves the focus, so putting the selection back preempts the focus change that would otherwise let `onFocus` steal the cursor. Making the non-cursor thoughts non-editable to stop the focus moving was tried and removed — it prevented nothing the restore did not already prevent, and it forced the mousedown handler to promote a thought to an editing host before placing the caret, because the browser silently drops a caret placed in a non-editable element.

**Known limitation: a caret is still *seen* travelling away from the insertion point and back.** It stays on the cursor thought's row — it does not visit other thoughts, so the restore above is doing its job — but it drifts horizontally with the finger, including well past the end of the text into empty space. That last detail is what identifies it: a thought of one character has only two text positions, so a caret drawn forty pixels beyond the glyph does not correspond to any position in the document, and therefore is not rendering the browser selection. Dragging right confirms it from the other side — the selection stays clamped to the end of the thought and never leaves the editable, so the restore above does not run, yet the caret still visibly drifts out. iOS draws it in its own text-interaction layer, composited above the web content, where page CSS cannot reach it.

Three further results agree. Adding `user-select: none` to the non-cursor thoughts stops the selection escaping at all — 56 consecutive `selectionchange` events all reported the cursor thought — and the artifact continues regardless. `caret-color: transparent` set on the document element hides the *real* caret and leaves this one untouched. Restoring the selection synchronously rather than a frame later makes no difference. Note that none of this reproduces in the Simulator, which does not drag the caret out of its editing host at all; a real device does, on iOS 17 as well as 26, so it is a simulator/device difference rather than a version one.

Note that `user-select` is not an option here: per CSS-UI-4 an editing host with `user-select: none` should behave as `contain`, but WebKit does not implement that clause and the trackpad crosses the boundary regardless.
...
- **The iOS autocomplete focus retarget** — after `insertReplacementText`, focus is bounced to the [`asyncFocus`](#asyncfocusts) dummy input and straight back to the same editable to clear the touch dead zone ([issue #4222](https://github.com/cybersemics/em/issues/4222)). Its `relatedTarget` is a bare `<input>`, so the check above cannot see it; `suppressBlurSync` in the [`editableSync`](../src/stores/editableSync.ts) store, which the retarget sets for the duration, is what marks it.
...
`updateCommandState` derives it three ways. When the cursor thought is empty and is holding a [pending format](#pending-format), it reports that formatting, since the thought's own value cannot carry it. With an active selection on a thought it asks the browser through `document.queryCommandState`, which reports formatting that has been enabled but not yet typed. With no selection — the usual state on mobile once the keyboard is down — it parses the value of each thought in [`selectedPaths`](../src/selectors/selectedPaths.ts), or the formatting an empty one is holding, and reports only formatting that covers all of them in their entirety. Deriving it from the selected paths rather than the cursor is what keeps the toolbar correct for a multiselection that has no cursor at all, which is what the Home button leaves behind.

It is refreshed from the `selectionchange` handler in [`initEvents`](../src/util/initEvents.ts), from [`updateUrlHistory`](../src/redux-middleware/updateUrlHistory.ts) whenever the cursor moves or the cursor thought's value changes, and by [`formatSelection`](../src/actions/formatSelection.ts) itself after it colors a multiselection. The latter two triggers cover what `selectionchange` cannot: with no focused editable the browser fires no selection event, so a value the user did not type — undo, redo, or any other programmatic replacement — would otherwise leave the toolbar describing formatting the thought no longer has.

### Pending format

`Thought.pendingFormat`, written by [`setPendingFormat`](../src/actions/setPendingFormat.ts).

Formatting that has been applied to an *empty* thought. An empty thought's value must stay empty — emptiness is what makes it a placeholder, and what Backspace and the bullet key off — so a color or a bold applied to it has nowhere to live in the value. [`formatSelection`](../src/actions/formatSelection.ts) accumulates it on the thought instead, and `Editable`'s `onChangeHandler` transfers the tags onto the first text typed into that thought with [`applyOuterTags`](../src/util/applyOuterTags.ts). Since the wrapped value differs from what was typed, the edit takes the immediate, forced branch, so the editable re-renders with the formatting and the browser carries it through the rest of the typing — the same mechanism that re-applies a cleared thought's formatting (#3673).
The formatting is stored as an ordinary thought value wrapping a single placeholder character, e.g. `<font color="#00d688">x</font>`, so that it composes through the same utilities as a real value: `formatSelectionHtml` applies further commands to it (replacing a color, toggling a bold back off), `getCommandState` derives the toolbar, bullet, and placeholder state from it, and `applyOuterTags` transfers its wrappers onto the typed text. Styling the placeholder reuses the `--placeholder-color` and `data-placeholder-*` machinery that shows a cleared thought's formatting, so the empty thought previews the formatting its text will take.

It lives on the thought rather than in a ministore because it has to behave like the formatting of a thought that has text. `setPendingFormat` is undoable, so each color is its own undo step, and undoing the first typed character restores the empty thought *with* the formatting it was holding — both fall out of `Thought` already being in the undo patch. Deleting the thought deletes the formatting with it, so no entry can outlive its thought. `editThought` drops it whenever the value changes, since the value then carries the formatting itself; that covers every path that writes a value, not just typing.

Being on the thought also means it is *not* cursor-scoped: the formatting survives navigating away from the empty thought and back, and formatting a second empty thought does not disturb the first. A multiselect follows from the same property — a color applied to several thoughts at once is held by every empty one among them, alongside the value edits that color the ones with text. `Editable` and [`Bullet`](../src/components/Bullet.tsx) therefore read it from the thought rather than through the cursor-scoped `commandStateStore`, so the placeholder and the bullet keep the color once the thought is no longer the cursor. It is in-memory only: `ThoughtPayload` ([`payload.ts`](../src/data-providers/treecrdt/payload.ts)) is an allowlist that does not include it, and `setPendingFormat` writes with `local: false, remote: false`, so holding formatting neither persists nor syncs. It does pass `overwritePending`, because holding formatting is not an edit and so does not advance `lastUpdated`, which `updateThoughts` would otherwise read as a stale echo from another device and discard.

A cleared thought and a pending format both feed the same two consumers, and a thought can be in both states at once, so the cleared value wins *only when it has one*: `clearThought` requires nothing but a cursor, and a multiselect clear sets the flag for every selected thought, so an empty thought can be cleared too. It has no tags of its own to re-apply, so both `onChangeHandler` and the placeholder fall through to the pending format rather than letting the cleared branch swallow it.

#### Notes

An empty *note* holds its formatting the same way, because a note is a thought: `toggleNote` creates the note's thought with an empty value, which is what gives the formatting somewhere to live. [`noteThought`](../src/selectors/noteThought.ts) resolves it, `formatSelection` writes to it under `noteFocus` instead of the cursor thought, and `Note`'s own `onChange` performs the transfer that `onChangeHandler` performs for a thought. A note whose text is assembled from several thoughts (`=children/=note/=path`) has no single thought to hold the formatting, so `noteThought` returns null and the command is a no-op there.

Two things differ from a thought. The held value is accumulated against `fgNote` rather than `fg`, since a note is semi-transparent by default and the two differ only in opacity (#3902, #4657). And the bullet must *not* follow the note: `commandStateStore` describes the caret, which under `noteFocus` is in the note, so `Bullet` skips its `commandStateStore` fallback there — the bullet belongs to the thought, and a note has no bullet of its own.

Only the colors preview on a note's placeholder. `Note` renders `react-contenteditable` directly, which re-renders only when one of the props it watches changes; `style` is one, so `--placeholder-color` reaches the DOM, but the `data-placeholder-*` attributes that drive bold, italic, underline, strikethrough, and code are not watched and never land. The marks still apply to the text once it is typed. Moving `Note` onto the app's own [`ContentEditable`](../src/components/ContentEditable.tsx), which spreads props without that gate, would fix it.
...
The exception is a behavior the harness's browser cannot exhibit at all. It collapses a touch selection on blur, unlike the mobile browsers em ships to, so a regression that depends on the selection outliving a blur — the text selector surviving a dismissed keyboard ([#4833](https://github.com/cybersemics/em/issues/4833)) — passes there whether or not the bug is present, and an e2e test would only appear to cover it. Creating a range and clearing it are faithful enough in JSDOM to hold that case, so it lives in [`Content`'s unit test](../src/components/__tests__/Content.ts) instead.
