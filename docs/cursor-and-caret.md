...
Two mechanisms take that selection with the keyboard. `Content`'s empty-space tap handler clears it on tap **down** rather than on the completed tap: waiting for the tap up leaves the context menu on screen while the keyboard animates away, so it visibly lingers and blinks before disappearing. That covers only the taps the page actually receives, so `dismissKeyboard` is the backstop — the keyboard hiding is the authoritative signal that the user is done editing, so it clears a live range whatever state edit mode is in. The extra condition matters because edit mode has usually _already_ ended by the time the platform reports the hide: the tap's blur ends it, as does a long press that selects a word (see `Editable`'s `onFocus`), which would otherwise leave `dismissKeyboard`'s edit-mode branch inapplicable and the selection stranded. A collapsed caret is left alone in both places: the blur dismisses it anyway, and clearing would blur the editable that a tap on a bullet is about to move the caret into.
