...
Timing is therefore what each mechanism is chosen for. `Content` collapses a range on empty-space tap **down**, ahead of the browser's default blur. On Android 11 and later, its `mousedown` capture handler also raises the fixed-control offset by the navigation inset immediately before that blur, without canceling the tap or its focus change. The Android Capacitor [keyboard handler](../src/device/virtual-keyboard/handlers/androidCapacitorHandler.ts) collapses on the tracker's `willHide` event, covering the Down Arrow and back button, and dismisses the caret on `didHide`. Older Android versions use Capacitor's `keyboardWillHide` and `keyboardDidHide` events. `didHide` is too late for range collapse: by then the menu has already flashed. Mobile web has no will-hide signal — the Chromium `geometrychange` event reports a *hidden* keyboard (`boundingRect.height === 0`) only once the animation is over, and reports it just once, so the [Android web handler](../src/device/virtual-keyboard/handlers/androidWebHandler.ts) collapses the range on that event and defers the blur past the next paint ([#5259](https://github.com/cybersemics/em/issues/5259)). `dismissKeyboard` remains the backstop that clears a live range whatever state edit mode is in, since edit mode may already have ended by the time the platform reports the hide. A collapsed caret is left alone on tap down so the next thought can receive it.

On Android 11 and later, [`VirtualKeyboardTracker`](../android/app/src/main/java/com/emtheapp/em/VirtualKeyboardTracker.java) reads each IME animation's interpolated fraction and multiplies it by the keyboard height from `WindowMetrics`. The WebView's raw IME inset can differ from the visible keyboard top on Samsung devices, so the independent height measurement prevents a persistent offset. The tracker sends per-frame CSS-pixel heights to a CSS custom property used by [`usePositionFixed`](../src/hooks/usePositionFixed.ts), without a React render or a guessed animation curve. The height includes the navigation inset, so the fixed-position calculation normally uses the greater of that height and the safe-area bottom instead of adding both. When the editable blurs before a hide, however, Android's visible IME surface rises by another navigation inset that neither the raw IME inset nor the animation bounds report. The handler latches that focus-loss path at `willHide` and adds the measured navigation inset for that hide only; a Down Arrow or back-button hide with the editable still focused does not receive it. The WebView paints CSS roughly one frame after the native keyboard, so the handler projects the latest native height 16 ms ahead using successive samples, clamped between zero and the measured shown height. It smooths the velocity between samples because projecting a single sample's velocity can make the fixed control briefly move backward while the keyboard continues forward; the measured height and lifecycle endpoints remain exact. It uses timestamps captured with those native samples, because a burst of bridge events can arrive within the same JavaScript millisecond and make a velocity based on JS receipt times spike. [`capacitor.config.ts`](../capacitor.config.ts) disables Android `SystemBars` inset handling: Capacitor otherwise pads the decor view by the full IME height and physically shrinks the WebView, so fixed controls receive that height twice and the viewport jumps to full size at the start of the keyboard close. [`MainActivity`](../android/app/src/main/java/com/emtheapp/em/MainActivity.java) still strips the IME inset sent to the WebView container. With a stable WebView viewport, an empty-space tap can blur the thought immediately while fixed controls follow the native keyboard. The handler only updates stores at keyboard lifecycle milestones. Android versions below 11 keep the existing Capacitor event behavior.
