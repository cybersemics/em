...
- **Split helpers:** `split(el)` and `splitNode(root, range)` return the HTML before/after the caret with formatting tags re-balanced. Used by the Split Sentences command, and by the Extract Subthought and Extract Category commands through [`splitFormattedValue`](../src/util/splitFormattedValue.ts), which they share to cut a formatted value into the part that stays and the part that is extracted.
