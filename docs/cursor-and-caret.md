...
Only one element is padded at a time, and `preventAutoscrollEnd` ignores any other element. `useEditMode` registers a focus listener per editable that ends the window as soon as focus has fired, so a cleanup queued by the thought that was focused a moment ago can run after `preventAutoscroll` has already padded the next one — creating a thought that the sort condition places away from the cursor moves focus to the moved thought and then to the new thought, which was mounted in the same render and has no focus listener yet to clean itself up. Without the identity check the stale call would cancel the 10 ms timeout, and a viewport-tall padding would stay on the focused editable as a blank area below it ([#4101](https://github.com/cybersemics/em/issues/4101)).
