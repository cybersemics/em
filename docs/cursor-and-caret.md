...
Timing is therefore what each mechanism is chosen for. `Content`'s empty-space tap handler collapses on tap **down**, ahead of the tap's own blur. That covers only the taps the page actually receives — in the Capacitor app the WebView's selection controller consumes the tap, and the Down Arrow is not a tap on the page at all — so the Android Capacitor [keyboard handler](../src/device/virtual-keyboard/handlers/androidCapacitorHandler.ts) publishes the `closing` phase on `keyboardWillHide`, the last signal that arrives while the keyboard is still up. The editor-owned [`initKeyboardSelection`](../src/device/initKeyboardSelection.ts) observer collapses the range at that phase and dispatches dismissal at `closed`; the metrics handler does not manipulate selection. `keyboardDidHide` is too late: it does not fire until the keyboard has finished animating away, and by then the menu has already flashed. Mobile web has no will-hide signal at all — the Chromium `geometrychange` event reports a *hidden* keyboard (`boundingRect.height === 0`) only once the animation is over, and reports it just once, so the [Android web handler](../src/device/virtual-keyboard/handlers/androidWebHandler.ts) has to make the gap itself: it collapses the range on that event and defers the blur past the next paint. Collapsing dismisses the menu on its own, and letting that reach the compositor before the focus goes is what keeps Android from rebuilding the menu and flashing a second, read-only one back ([#5259](https://github.com/cybersemics/em/issues/5259)). `dismissKeyboard` remains the backstop that clears a live range whatever state edit mode is in, since edit mode has usually _already_ ended by the time the platform reports the hide — the tap's blur ends it, as does a long press that selects a word (see `Editable`'s `onFocus`), which would otherwise leave its edit-mode branch inapplicable and the selection stranded. A collapsed caret is left alone throughout: the blur dismisses it anyway, and clearing would blur the editable that a tap on a bullet is about to move the caret into.

On Android 11+, [`AndroidKeyboardPlugin`](../android/app/src/main/java/com/emtheapp/em/AndroidKeyboardPlugin.java) supplies the system's animation curve, scaled duration and a Choreographer clock anchor. It sends actual endpoint changes and completion, not per-frame positions. The [Android Capacitor handler](../src/device/virtual-keyboard/handlers/androidCapacitorHandler.ts) calibrates native and JavaScript clocks, samples that curve locally on JavaScript animation frames, and publishes intermediate `height`, visibility and lifecycle `phase` through `virtualKeyboardStore`. Like iOS, height is normalized above the navigation safe-area baseline. [`usePositionFixed`](../src/hooks/usePositionFixed.ts) consumes only the shared store and uses the same rendering path on Android; there is no Android DOM animation, anchor registry, ref or custom CSS-property path. Older Android reports its available lifecycle endpoints through that same store.

[`KeyboardWebView`](../android/app/src/main/java/com/emtheapp/em/KeyboardWebView.java) preserves only the departing editor's input type and IME options when Chromium disconnects its input connection. Some keyboards otherwise change layout during the close—for example, adding a number row—while Android is still animating the original height. Retaining the layout hints avoids that mid-close size change. The editable blurs immediately, the connection remains null, and each new editor supplies fresh hints; the rule does not identify particular keyboards.

The IME inset is consumed before reaching the WebView container because em handles keyboard avoidance itself. Capacitor SystemBars inset padding is also disabled, keeping Android from additionally resizing or moving the viewport. Keyboard geometry remains available to the native plugin above that container. This gives the application one keyboard state source and one rendering path.
...
- [`cursorBack`](../src/commands/cursorBack.tsx) (Escape) exits multi edit mode — restoring the cleared values and closing the keyboard — while leaving the multiselection intact. A second Escape then clears the multiselection, so the two are undone in the order they were applied. The command's Clear Thought guard checks `isMultiEditing` in addition to `cursorCleared`, since the first edit resets the cleared state (see `editThought`) while the multiselection is still being edited.
