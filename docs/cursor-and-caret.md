...
[Clear Thought](../src/commands/clearThought.ts) works on a multiselection: it clears every selected thought and keeps the multicursors alive so that subsequent typing is mirrored to all of them by `Editable`'s `onChangeHandler`, which dispatches an `editThought` per selected thought on every keystroke rather than through the edit throttle, so that they stay in sync character by character. The multicursors are keyed by path, and `onChangeHandler` is memoized so that `ContentEditable` is not re-rendered on every keystroke, so the handler recognizes the thought it is typing into by the current path held in a ref rather than by the one its closure captured — otherwise a command that moves the selection before it is cleared, such as Indent, would leave every thought but the one being typed into unedited. Only one thought can hold the real browser caret, so the others render a faux caret to show that they are being edited too.
