...
`updateCommandState` derives it two ways. With an active selection on a thought it asks the browser through `document.queryCommandState`, which reports formatting that has been enabled but not yet typed. With no selection — the usual state on mobile once the keyboard is down — it parses the value of every thought a formatting command would act on: the multiselection when there is one and the cursor otherwise ([`selectedPaths`](../src/selectors/selectedPaths.ts)), reporting only formatting that covers the whole value of all of them. Reading the selection rather than the cursor is what lets the toolbar describe the thought the user actually selected: one long pressed while the cursor is still on the thought that was formatted last, or one held after the Home button has dismissed the cursor entirely. With nothing selected, no formatting can apply, so the state is reset instead of left describing the thought that was formatted last.

It is refreshed from the `selectionchange` handler in [`initEvents`](../src/util/initEvents.ts), and from [`updateUrlHistory`](../src/redux-middleware/updateUrlHistory.ts) whenever the selection changes or the cursor thought's value changes. The second trigger covers what `selectionchange` cannot: with no focused editable the browser fires no selection event, so a value the user did not type — undo, redo, or any other programmatic replacement — would otherwise leave the toolbar describing formatting the thought no longer has. The multicursors are compared by identity there, since the reducers that write `state.multicursors` only replace it when the selection changes. The refresh is deferred while `isMulticursorExecuting` is set, so that a multicursor command, which restores its selection one thought at a time, recomputes the state once at the end rather than once per thought restored. [`formatSelection`](../src/actions/formatSelection.ts) refreshes it directly after coloring a multiselection, since those per-thought edits change neither the cursor nor the multicursors.
