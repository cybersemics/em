...
Within a thought that is already being edited, the caret is moved by the tap's synthesized `mousedown`, which `useEditMode` resolves to a character offset with [`getCaretOffset`](../src/device/getCaretOffset.ts). WebKit withholds that mousedown when the tap lands within a few characters of the caret — it reads the tap as an interaction with the selection UI rather than as a new caret placement — and it also undoes the caret it briefly placed there itself, some 26–39 ms after the `touchend`. Neither half is observable in the web layer: no mouse event is delivered at all, and the revert is performed natively rather than through any selection API. The tap is therefore swallowed whole, which is what made a word four or five characters from the caret unreachable ([issue #4220](https://github.com/cybersemics/em/issues/4220)). `useEditMode`'s `touchend` handler resolves the tapped offset through the same `getCaretOffset` the mousedown path uses and re-checks the caret against it once the revert would have happened, correcting it only if WebKit left it elsewhere. A tap WebKit handled normally is thereby left alone, as is a range — a long press or double tap selects text, and collapsing it would be the same kind of hijack `allowDefaultSelection` exists to avoid. Deferring past the revert is the whole point of the mechanism, so the wait has to clear the late mousedown too (37–42 ms), and a `touchstart` cancels anything still pending on the same reasoning as the listeners above: a new touch proves the previous tap's correction no longer applies.
