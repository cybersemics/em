...
A blur that does end editing also trims the thought's value. Typed edits are trimmed on their way to Redux by `Editable`'s change handler, but a paste never passes through it — [`useOnPaste`](../src/components/Editable/useOnPaste.ts) dispatches [`importData`](../src/actions/importData.ts) instead, which preserves the whitespace of pasted HTML. Trimming at the blur rather than at the import is what lets the pasted whitespace stay visible in the editable while the user is still editing, as typed whitespace does, and still keeps it out of the persisted value ([issue #5232](https://github.com/cybersemics/em/issues/5232)). A transient editable is exempt, since it has no thought to edit and the change handler would create one.
