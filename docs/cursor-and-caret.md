...
- **Split helpers:** `split(el)` and `splitNode(root, range)` return the HTML before/after the caret with formatting tags re-balanced. Used by the Split Sentences command, the Extract Subthought command, and by paste to cut out the text a selection replaces. `collapsedRangeAtOffset(root, offset)` is the mapping the last two are built on: it turns a plain-text offset into a collapsed `Range` inside `root`, and returns null when `root` has no text to resolve against.
