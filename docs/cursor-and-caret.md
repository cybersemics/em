...
A horizontal swipe must not move the caret either. Chrome Android's swipe-to-move-cursor drags the caret of the focused thought whenever a horizontal pan begins on *any* editable — including a different thought, and including a gesture — clamping the finger into the focused thought's line box and raising the magnifier over it ([#4272](https://github.com/cybersemics/em/issues/4272)). The thoughtspace never pans horizontally, so [`editableRecipe`](../src/recipes/editable.ts) declares `touch-action: pan-y` on the editable, which keeps the browser from recognizing that pan at all while leaving vertical scrolling and em's own gestures (driven by raw touch events) untouched. That covers the swipe wherever it starts, including the [scroll zone](commands.md#gesture-activation), where thoughts extend underneath the strip: the zone excludes gestures and scrolls the thoughtspace, but it does not intercept touches, so a tap on a word underneath it still places the cursor in that thought.
