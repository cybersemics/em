...
- **A stranded selection is pulled back.** Making the neighbours non-editable stops the *focus* from moving but not the *selection*, which the trackpad can still drop into non-editable text. A `selectionchange` listener in `useEditMode` restores the caret to the cursor thought's editable when the selection has left it. It runs only while that editable is still `document.activeElement`, which is what distinguishes this from the many flows that legitimately move the selection away after blurring it first (`selection.clear`, the Command Center, drag start), and only when no touch has reached an editable recently — a tap is the user moving the caret on purpose, including into a thought that is about to become the cursor. The restore is synchronous: `selectionchange` is dispatched before the frame paints, so correcting the selection inside the handler means the stray position is never rendered and the caret is not seen flying to another thought and back. Deferring it even one `requestAnimationFrame` is enough to make that flight visible. A per-frame attempt cap stands in for the deferral as the guard against a browser that refuses to move the selection back.
