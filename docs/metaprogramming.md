...
- **`=bullet`** — controls the bullet display. Options: `None` (hide the bullet); `Ordered` (render a 1-based ordinal number in place of the bullet, so children display as a numbered list); `Alpha` (render a 1-based letter — `a.`, `b.`, `c.`, … — so children display as a lettered list); `Time` (render a clock time in place of the bullet, so children display as a schedule — see below). Read by [`useHideBullet`](../src/hooks/useHideBullet.ts) and [`Bullet`](../src/components/Bullet.tsx). Replaces the old `=bullets` attribute. Also propagable via `=children/=bullet` / `=grandchildren/=bullet` — e.g. `=children/=bullet/Ordered` numbers every direct child. The bullet style of the current list can also be set from the toolbar via the **Bullet Style** picker ([`toggleBulletPicker`](../src/commands/toggleBulletPicker.ts)), which writes `=children/=bullet` on the cursor's parent.
  - **`=bullet/Time`** takes an optional step as its child — `5min`, `15min`, `30min`, or `1h`, defaulting to `15min` — e.g. `=children/=bullet/Time/30min`. Times are derived forward through the list by [`getBulletTime`](../src/selectors/getBulletTime.ts): the first thought starts at the `created` timestamp of the `Time` thought (i.e. when the style was applied) rounded up to the step, and each following thought is the previous time plus the step, wrapping past midnight. Because the start is the `Time` thought's `created` timestamp, [`setBulletStyle`](../src/actions/setBulletStyle.ts) recreates the `=bullet` subtree when switching another style to `Time`, and only rewrites the step once the list is already `Time`. Times render in the locale's clock, or in the clock of the list's `=stepStart` literals once one exists. Tapping a Time bullet opens a popover ([`TimePicker`](../src/components/TimePicker.tsx), toggled by [`toggleTimePicker`](../src/actions/toggleTimePicker.ts) and anchored via `state.timePickerPath`) instead of moving the cursor; it offers a list of times at the step, the step options, and **Clear time**. Selecting **Times** in the Bullet Style picker applies the default step and keeps the picker open with a second row of step options.
- **`=stepStart`** — a clock time literal on a thought in a `=bullet/Time` list (`1:30`, `13:30`, `1:30 pm`), which re-bases that thought and every thought below it; on the first thought it sets the start of the list, and mid-list it expresses a break or gap. Out-of-order values are allowed and not validated; an unparseable value is ignored. Written by the Time bullet popover, and removed by its **Clear time** option.
