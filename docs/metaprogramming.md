...
- **`=focus`** — when the cursor lands on this thought, change how it is rendered. Options: `Normal` (default), `Zoom`. The attributes under `Zoom` (e.g. `=focus/Zoom/=style`, `=focus/Zoom/=styleContainer`, `=focus/Zoom/=bullet/None`) are applied while the cursor is on the thought or one of its descendants, resolved through [`findFirstEnvContextWithZoom`](../src/selectors/findFirstEnvContextWithZoom.ts) so that a `=let` binding can define them. Zoom mode was also intended to hide the parent and siblings to give the active thought the full screen; [`useZoom`](../src/hooks/useZoom.ts) still computes that, but nothing calls it and [`calculateAutofocus`](../src/selectors/calculateAutofocus.ts) does not consider `=focus`, so the hiding does not currently happen.
