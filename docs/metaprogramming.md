...
- **`=focus`** — when the cursor lands on this thought, change how it is rendered. Options: `Normal` (default), `Zoom`. Zoom is only partially implemented: `=focus/Zoom/=styleContainer` on the thought itself is applied by [`useThoughtStyleContainer`](../src/hooks/useThoughtStyleContainer.ts), `=focus/Zoom/=bullet/None` by [`useHideBullet`](../src/hooks/useHideBullet.ts), and the `=let` form of `=focus/Zoom/=style` by [`Subthought`](../src/components/Subthought.tsx); the `=let` forms are resolved through [`findFirstEnvContextWithZoom`](../src/selectors/findFirstEnvContextWithZoom.ts). The hiding of parent and siblings that gives Zoom its name is **not** implemented: [`calculateAutofocus`](../src/selectors/calculateAutofocus.ts) never consults `=focus`, and [`useZoom`](../src/hooks/useZoom.ts), which computes it, has no callers.
