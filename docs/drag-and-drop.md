...
A touch that lands on the caret never becomes a long press at all: the capture-phase touchstart listener in [`initEvents`](../src/util/initEvents.ts) latches [`globals.pressOnCaret`](../src/globals.ts), and `start` reads it and simply does not mark the press, so the `dragStart` event finds nothing pressed and none of the downstream effects — haptics, the scroll lock, `DragHold` — run. That press belongs to native caret repositioning — the iOS magnifier, the Android caret handle — which the user is reaching for to move the caret ([issue #3763](https://github.com/cybersemics/em/issues/3763)). The check requires the point to fall *inside* the editable, so a press on the bullet still starts a drag even when the caret sits at the start of the text, and a press elsewhere on the thought's text is far enough from the caret to drag as usual. See [Cursor and Caret](cursor-and-caret.md#selectionrangestore).
