...
A touch does not start a long press at all while [`touchStore`](../src/stores/touch.ts)`.touchEndUnreliable` is set. After a double tap on the caret of an editable, iOS 27 stops dispatching `touchend` and `pointerup` when a later tap that leaves the caret where it is lifts, sends no `touchcancel` or `pointercancel` either, and flushes the withheld `touchend` immediately before the next `touchstart` ([#5660](https://github.com/cybersemics/em/issues/5660)). A tap that moves the caret or focus is delivered on time, and the state survives blur and refocus. Until the flush, the withheld tap is indistinguishable from a finger still held down: `:active`, focus, `selectionchange` and `click` offer no reliable signal at the long-press delay. Without the guard, the backend's timer fires `dragStart` on a lifted finger and the next touch ends the false `DragHold`, toggling the multicursor. Since the lift cannot be detected, the capture-phase touch listeners in [`initEvents`](../src/util/initEvents.ts) detect the conditions under which a `touchend` will be withheld instead. iOS is known to be stuck once a `touchend` has arrived within `TOUCHEND_WITHHELD_MS` of the next `touchstart`, which no real lift and re-touch can do, and that is remembered for the life of the page. On WebKit the touch right after a double tap is treated the same way, since the first withheld tap has no withheld `touchend` before it to give it away. In either case only a touch that lands on the word holding the caret ([`selection.isOnCaretWord`](../src/device/selection.ts)) is unreliable, because only that tap leaves the caret in place. The cost is that a genuine long press on the caret's word does not activate `DragHold` once iOS has entered this state; a long press anywhere else still does.
...
When a touch drag ends, application cleanup returns `state.longPress` to `Inactive` immediately so scrolling, gestures, alerts, and multicursor cleanup are restored without a timer. Before that transition, `useDragAndDropThought` sets `suppressCursorAfterTouch` in the [`touch`](../src/stores/touch.ts) store. `Editable` ignores cursor-producing compatibility events while the flag is set, and the next capture-phase `touchstart` clears it. This ties suppression to the completed touch gesture rather than assuming that the browser will dispatch its trailing click or focus before a zero-delay timer.
