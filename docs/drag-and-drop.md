...
- **`canDrag`** rejects drags from immovable / readonly thoughts (checked via `=immovable` / `=readonly` attributes on the thought *or its parent*) and from non-editable documents. On touch it additionally requires `state.longPress === DragHold`: every touch drag is preceded by a long press that sets it, so a press that never reached `DragHold` — one on the caret, or one made while a text range is selected — cannot start a drag when it later moves past the touch slop. The backend's long-press timer fires independently of `canDrag`, so this gate and the one in `useLongPress` are both needed.
...
A touch that lands on the caret never becomes a long press at all: `start` checks [`selection.isCaretNear`](../src/device/selection.ts) and simply does not mark the press, so the `dragStart` event finds nothing pressed and none of the downstream effects — haptics, the scroll lock, `DragHold` — run. That press belongs to the iOS text magnifier, which the user is reaching for to reposition the caret ([issue #3763](https://github.com/cybersemics/em/issues/3763)). The check requires the point to fall *inside* the editable, so a press on the bullet still starts a drag even when the caret sits at the start of the text, and a press elsewhere on the thought's text is far enough from the caret to drag as usual. See [Cursor and Caret](cursor-and-caret.md#selectionrangestore).
