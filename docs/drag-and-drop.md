...
A touch does not start a long press at all while [`touchStore`](../src/stores/touch.ts)`.touchEndUnreliable` is set. After a double tap on the caret of an editable, iOS 27 stops dispatching `touchend` and `pointerup` when a later tap lifts, sends no `touchcancel` or `pointercancel` either, and flushes the withheld `touchend` immediately before the next `touchstart` ([#5660](https://github.com/cybersemics/em/issues/5660)). Until then the tap is indistinguishable from a finger still held down: `:active`, focus, `selectionchange` and `click` offer no reliable signal at the long-press delay. Without the guard, the backend's timer fires `dragStart` on a lifted finger and the next touch ends the false `DragHold`, toggling the multicursor. Since the lift cannot be detected, the stuck state is detected instead, in the capture-phase touch listeners in [`initEvents`](../src/util/initEvents.ts): a touch is unreliable when the previous `touchend` arrived within `TOUCHEND_WITHHELD_MS` of its `touchstart`, which no real lift and re-touch can do, or, on WebKit, when the two touches before it were a double tap, since the first withheld tap has no withheld `touchend` before it to give it away. A touch whose `touchend` arrives on time clears the state for the touch after it. The cost is that a genuine long press made while iOS is in this state does not activate `DragHold`.
