...
Three consequences for anyone extending the pipeline:
...
- **Prefer a parser over a regex.** The funnel itself already depends on [himalaya](https://github.com/andrejewski/himalaya) for HTML and DOMPurify for sanitization, yet much of its transformation is done by regex — usually *ahead* of the parse, deciding what the parser is about to establish anyway. [`htmlToJson`](../src/util/htmlToJson.ts) shows the cost most plainly, because there the parse has already happened: it parses with himalaya to discover which tags are present, converts that list into a regex, applies the regex to the same raw string, then parses the result a second time. The round trip is where correctness is lost — `</?(?:…)>` cannot match a tag carrying attributes, which is why `<p class="p1">` survives a filter meant to drop `<p>`, and why the ChatGPT branch that keys on that class works at all. Elsewhere the remedy is ordering rather than substitution: [`REGEX_STARTS_WITH_CLOSED_TAG`](../src/util/textToHtml.ts) (whose own comment concedes it "cannot check properly for a tag nested within itself") and the `<li` count in [`importText`](../src/actions/importText.ts) both decide, before parsing, what parsing would settle, and [#5175](https://github.com/cybersemics/em/issues/5175)'s parse-then-route reduces the second to `blocks.length`. Markdown is the sharpest case, because the real parser is present but out of reach: [`marked`](https://github.com/markedjs/marked) is used only in [`markdownToText`](../src/util/markdownToText.ts), upstream of the funnel and reachable only through `importText`'s markdown branch, while `REGEX_MARKDOWN_BOLD`/`REGEX_MARKDOWN_ITALICS` do markdown parsing *inside* the funnel on every import. They do not implement CommonMark's flanking rules, so where `markdownToText` correctly leaves `2 * 3 * 4` alone, `textToHtml` rewrites it to `2 <i> 3 </i> 4` — and on the markdown path both run, with the non-conformant one last. [#5172](https://github.com/cybersemics/em/issues/5172) moving markdown conversion into the shared funnel is what makes deleting the two regexes possible. Regex keeps its place on genuinely unstructured text (`REGEX_PLAINTEXT_BULLET` matching a leading bullet glyph) and on a parser's own output when it is correcting that parser (`REGEX_NBSP` in [`strip`](../src/util/strip.ts) must run after DOMPurify, which introduces the entity).
