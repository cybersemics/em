...
- In shadow mode on the sample pull requests in U10, a validated rename lands with high footprint and low substantive scores, a rename carrying one unrelated behavioral edit keeps that edit visible in its file row, and a tiny change establishing a shared abstraction scores high on future commitment.
...
- KTD4. **Mechanical classification uses the tooling the repository already carries.** Comment-only and identifier-rename checks compare TypeScript token streams with trivia skipped (the compiler API is a root dependency); formatting-only checks compare prettier-normalized blobs under the repository's own config; exact moves come from `git diff -M100%`. A rename is validated when one derived token mapping reproduces every classified edit exactly; anything it does not reproduce stays residual. Tooling comments (`@ts-ignore`, `@ts-expect-error`, `eslint-disable`) disqualify a comment-only classification. Externally meaningful names touched by a rename (exported symbols, string literals equal to the old name, object property keys, paths under `src/data-providers/`) are listed as boundaries for the assessor.
- KTD5. **The assessor runs as a headless Claude Code session driven from the workspace, with tools restricted to reading and read-only git.** The skill at `.github/skills/pr-risk-assessment/SKILL.md`, the rubric, and the policy config are passed by absolute path from the trusted checkout; the session's working directory is the PR head checkout; project-level settings and instruction files are not loaded, so a CLAUDE.md or skill in the PR cannot steer the run. The session returns one JSON document that `zod` validates before anything is published. Alternative considered: a tool loop over the OpenAI Responses API, which the other two workspaces' plain `fetch` style would have to grow into; rejected for v1 because the harness gives retrieval, budgeting, and a tool boundary for free, and the adapter in U5 isolates the choice behind the change-package-in, result-JSON-out contract. See Assumptions.
...
- KTD7. **Publication is a check run on the head commit plus one marker-upserted PR comment.** A check run binds to the SHA, so a new push leaves the new head without a result and satisfies R3 with no bookkeeping; the comment carries the brief, the matrix, and a collapsed JSON block the reviewer-side script in U9 reads. The comment pattern mirrors `scripts/ci/upsert-diff-comment.cjs` and `scripts/ci/task-comment.cjs`.
- KTD8. **The trigger is a `workflow_run` completion gate in the shape of `pr-ready.yml`, armed by a review request.** A `pull_request: review_requested` event records the pending head on a marker comment; each CI workflow completion re-evaluates whether every check on that head is complete and green and whether the request still names that head; only the last one out runs the assessor. The workflow runs from the default branch by construction of `workflow_run`, which is what R22 needs.
...
- A formal review request means any `review_requested` event on the pull request. Copilot's session-end review request qualifies, which is the intended entry point for agent-authored PRs.
...
- **Approach:**
  1. Copy the workspace conventions from `scripts/issue-classifier/`: `"type": "module"`, `build` and `typecheck` as `tsc --noEmit`, `dotenv` and `zod` dependencies, Node 24 direct execution of `.ts`.
...
- **Approach:**
  1. Formatting-only: run prettier programmatically with the repository config on both blobs; equal output classifies the edit.
  2. Comment-only: scan both blobs with the TypeScript scanner, drop trivia, compare token kinds and text; equal streams classify the edit unless a tooling comment was added, removed, or changed, which keeps it residual (KTD4).
  ...
  6. Assemble the package: manifest, transformations with the files each accounts for, residual hunks, boundaries, mandatory-review matches, and a ledger seed marking transformation-covered files.
- **Patterns to follow:** `packages/ai/` for use of the TypeScript compiler API at runtime; `scripts/debugLogCompare.ts` for a pure comparison module with its own test file.
- **Test scenarios:**
...
- **Approach:**
  1. Write the change package to a private temp directory, then spawn the headless session with the working directory set to the PR head checkout, tools restricted to read, glob, grep, and read-only git subcommands, project settings and instruction files disabled, the skill, rubric, and package passed by absolute path, and a token budget from `config.json`.
  2. Extract the final JSON document from the session output, validate with `resultSchema`, and return either the result or a failure carrying the raw output for the check-run summary.
...
- **Test scenarios:**
  ...
  - Integration: the spawn arguments include the read-only tool list and exclude project setting sources.
...
- **Approach:**
  ...
  2. `github.ts` covers: get PR, list check runs for a ref with `filter: latest`, create and update a check run, list and upsert comments by marker, add and remove labels, and the GraphQL `markFileAsViewed` used by U9.
  3. `assess.ts` composes eligibility → sweep → runAssessor → routing → publish, with `--dry` printing what would be published and writing nothing.
  4. Check run: name `PR Risk`, bound to the head SHA; conclusion `neutral` in shadow mode, `success` for `skip`, `action_required` for `review`, `failure` for an execution failure with the raw output in the summary.
...
- **Test scenarios:**
  ...
  - Error path: an execution failure renders a check summary containing the failure reason and no matrix.
...
- **Approach:**
  1. Triggers: `pull_request` with types `review_requested` and `ready_for_review`, `workflow_run` on the CI workflows `pr-ready.yml` already lists, and `workflow_dispatch` with a PR number.
  2. On `review_requested`, record the requested head SHA on the marker comment (the pending state) and stop unless checks are already complete and green.
  3. On `workflow_run` completion, resolve the open PR for the head branch, read the pending head from the marker, bail unless it equals the current head, bail unless every latest check run is complete with no failed conclusion, bail if a `PR Risk` check already exists for that head (R4), then run `assess.ts`.
  4. Checkout: the default branch at the workflow's own ref for code, skill, rubric, and config; the PR head by SHA into `pr-head/` with full history so the merge base resolves.
  5. Concurrency group per PR with `cancel-in-progress: false`, mirroring `pr-ready.yml`; permissions `checks: write`, `pull-requests: write`, `contents: read`.
...
- **Test scenarios:**
  ...
  - Edge: a `PR Risk` check already exists on the head → not eligible, reason is duplicate.
...
- **Approach:**
  1. Resolve the PR, find the marker comment, parse the JSON block, and verify its `head` equals the PR's current head; refuse with a message if they differ (R3).
...
- **Patterns to follow:** `scripts/estimate/src/command.ts` for a small command entry with argument parsing and a dry flag.
- **Test scenarios:**
...
- The assessor session provably has no write tools and loads skill, rubric, and config from the default-branch checkout.
...
- **PR-controlled instruction files.** A pull request can add a `CLAUDE.md` or skill; the session must run with project setting sources disabled and cwd treated as data (R22). U5's integration scenario pins the spawn arguments.
- **`workflow_run` semantics.** Edits to `pr-risk.yml` take effect only after merging to main, and `workflow_run.pull_requests` is empty when the head moved, so the PR is resolved from the head branch as `mark-copilot-pr-ready.cjs` does.
- **Rename validation on non-TypeScript files.** Token-stream validation covers TypeScript only; renames touching CSS, JSON, or markdown stay residual and cost assessor attention. Acceptable for v1; the residual is still complete.
