...
- R24. Files at or below the threshold are marked viewed only through a token belonging to a requested reviewer of the pull request, and only when the pull request head still equals the assessed head.
...
- No check run is created on the head commit in this release (KTD9).
...
- **A check run** named `PR Risk` on the head commit, which needs `mark-copilot-pr-ready.cjs` and `collect-dependabot-failures.cjs` to ignore it by name first.

### Success Criteria

- On the fixture set in `.github/instructions/pr-risk/samples.jsonl`, `yarn workspace em-pr-risk evaluate` reports the expected route for every sample and no `skip-eligible` route on a sample labeled `human`.
- A shadow run on at least ten recent Copilot pull requests completes without an assessor failure caused by budget or schema, and a reviewer reading each brief can name where to start without opening the file list.
...
- KTD3. **"Full CI pass" is an algorithm over the head's latest check runs, not a fixed set.** Path filters mean a documentation-only pull request reports only Lint, and a conflicting pull request reports only the two `pull_request_target` workflows (`docs/testing.md`, Path filtering and Merging the base branch). The gate requires: a `Lint` check run present with conclusion `success`; every check run completed; none with conclusion `failure`, `timed_out`, `action_required`, or `cancelled`; `mergeable` not `false`, retried while GitHub reports it as unknown. The set of check names it saw is recorded in the state so the reviewer can see what "full" meant. This is stricter than `mark-copilot-pr-ready.cjs`, which tolerates `cancelled`; the shared module takes the tolerance as an option. Governs R14.
- KTD4. **Eligibility is a pure function** of the pull request, its timeline, its latest check runs, the existing state, the triggering event, and the policy. It is stateless except for the sticky comment. A review request is bound to the current head when the newest `review_requested` timeline event follows the newest `committed` event. Copilot authors additionally require the newest `copilot_work_*` event to be `copilot_work_finished` in place of a non-draft requirement, because the undraft happens concurrently in `PR Ready` and nothing fires after it. Human authors require non-draft. Dedupe is by head SHA plus merge-base SHA against the state. Governs R14, R16, R17.
- KTD5. **Extract the gate logic `pr-ready.yml` and `PR Risk` share into `scripts/ci/pr-gate.cjs`**: open pull request resolution by number or head branch, Copilot session state from the timeline, latest check-run enumeration, and the failed-conclusion set. `mark-copilot-pr-ready.cjs` keeps its behavior and requires the module. The repository's rule is one file rather than two that drift (`docs/agents/external-agents.md`, Why the skills are symlinked). CommonJS so `actions/github-script` can `require` it and ESM entry points can `import` it, as `scripts/ci/task-comment.cjs` does.
- KTD6. **Trust boundary.** The workflow runs from the base repository checkout in base context. The pull request head is fetched by SHA as git objects into the same repository and is never installed from or executed; the sweep and the assessor's tools read blobs through `git show` and `git grep` against the base and head tree-ishes. Rubric, policy, and sensitive-path lists are read from the base checkout. A pull request touching `.github/workflows/**`, `scripts/ci/**`, `scripts/pr-risk/**`, `.github/instructions/pr-risk/**`, `.github/skills/**`, `.github/agents/**`, `AGENTS.md`, `CLAUDE.md`, `.agents/**`, or `.claude/**` routes `mandatory-human`. (session-settled: user-approved — the read-only, evidence-not-instructions boundary.) Governs R19, R20.
...
- KTD9. **Output lives in one marker-keyed sticky comment plus a workflow artifact, with no check run.** The comment (`<!-- pr-risk -->`, state in `<!-- pr-risk-state: <base64url JSON> -->`) carries the brief, the pull request criterion vector, the summaries, routing and reasons, the head, base, and merge-base SHAs, the check names seen, the run link, and the artifact identifier. The full per-file matrix is the artifact `pr-risk-<headSha>` because a several-hundred-file matrix exceeds the comment size limit. A check run is excluded because `mark-copilot-pr-ready.cjs` enumerates every check on the head and an in-progress one would stop the undraft. Governs R10, R13, R15.
- KTD10. **Routing policy is data in `.github/instructions/pr-risk/policy.json`**: `mode`, the auto-view threshold, per-criterion skip thresholds, the informational criteria (footprint), mandatory-review globs, sensitive-surface globs, excluded authors, and label names. The controller emits `human`, `mandatory-human`, or `skip-eligible` with reasons. In shadow mode the only effects are the comment, the artifact, and the labels `risk: human`, `risk: mandatory-human`, `risk: skip-eligible`. (session-settled: user-approved — deterministic controller applying trusted policy; shadow first.) Governs R18, R19, R27.
- KTD11. **Auto-view is one module with two callers.** `scripts/pr-risk/src/view.ts` reads the artifact named in the state, verifies the head, selects rows with file score below the threshold, and calls the `markFileAsViewed` GraphQL mutation in batches. In CI it runs as a separate step with `PR_RISK_REVIEWER_TOKEN`, a fine-grained token belonging to one named reviewer (precedent: `PREVIEW_QR_TOKEN` in `docs/testing.md`), and applies only when that token's login is among the requested reviewers, otherwise reporting why not. Locally any reviewer runs it with their own `gh auth token`, and a shared skill documents the invocation. (session-settled: user-approved — viewer-specific state applied in the reviewer's context, separate from the backend.) Governs R24, R25, R28.
...
- New workflow file names must be added to the sibling-workflow `paths-ignore` lists in `test.yml`, `puppeteer.yml`, `ios.yml`, and `vercel-preview.yml`, and `scripts/pr-risk/**` to the Puppeteer and BrowserStack lists beside `scripts/estimate/**`.

### Sequencing

U1 and U2 are independent foundations. U3, U4, U5, and U6 build the pipeline in data-flow order and each is testable without the workflow. U7 wires the workflow and eligibility and is the first unit that runs in CI. U8 and U9 follow U7. U10 documentation rides with the units that make it necessary; its own unit exists for the new page and the cross-references. Shadow validation on real pull requests happens after U7 and before any enforce-mode work.
...
- **Approach:**
  ...
  2. Add an option for whether `cancelled` counts as failed, defaulting to the current `PR Ready` behavior, and an option to require a named check (`Lint`) with conclusion `success`.
  3. `mark-copilot-pr-ready.cjs` calls the module with its current options so its decisions are byte-for-byte unchanged.
...
- **Verification:** `yarn test scripts/ci` passes; a manual `gh workflow run pr-ready.yml -f pr=<n>` on an already-ready pull request logs "already ready for review" as before.
...
- **Requirements:** R1, R7, R8, R9, R12, R19, R20; KTD1, KTD10, KTD14.
- **Dependencies:** none.
- **Files:**
  - `scripts/pr-risk/package.json`, `scripts/pr-risk/tsconfig.json`, `scripts/pr-risk/README.md` (create; mirror `scripts/estimate/`)
...
- **Approach:**
  ...
  3. `policy.json` holds the fields KTD10 lists; `loadPolicy.ts` validates it with zod and rejects unknown criterion identifiers.
...
- **Approach:**
  ...
  4. `sweep.ts` returns `{ inventory, transformations, residual }` and asserts every inventory entry is in exactly one transformation's members or in `residual`.
...
- **Approach:**
  1. `buildPackage.ts` renders the manifest, each transformation with its members summarized (count, directory, boundary questions), and residual diffs inline while under the budget in the policy, otherwise as a list with sizes.
  2. `tools.ts` implements the four tools from KTD8 over `git show` and `git grep` on the base and head tree-ishes, with per-call output caps and a shared call counter; a call over the cap returns an error the session sees, and the session is failed when the total context budget is exceeded.
...
- **Approach:**
  ...
  2. `fanOut.ts` produces a row for every changed file: residual rows as scored, transformation members with the transformation's scores, and the previous path on renames.
...
- **Approach:**
  ...
  2. `parseState.ts` round-trips the base64url JSON with a schema version, tolerating an absent or older state.
  3. `upsertComment.ts` finds the marker comment, re-reads the pull request immediately before every write, and skips the write when the pull request is closed or the head has moved since the run started.
  4. `writeArtifact.ts` writes the assessment JSON to the runner's workspace for `actions/upload-artifact` and records the artifact name in the state.
...
- **Test scenarios:**
  ...
  - Upsert with an existing marker comment updates it; with none creates one; with an identical body skips the write.
...
- **Files:**
  ...
  - `.github/workflows/test.yml`, `puppeteer.yml`, `ios.yml`, `vercel-preview.yml` (add `.github/workflows/pr-risk.yml` to the sibling-workflow `paths-ignore` group)
...
- **Approach:**
  1. `decide.ts` implements the KTD4 decision from `{ pr, timeline, checkRuns, state, event, policy, dispatchOverrides }` and returns `{ action: 'assess' | 'stale' | 'reuse' | 'skip', reason }`, using `scripts/ci/pr-gate.cjs` for the session and check evaluation with the strict options from KTD3.
  2. `assess.ts` resolves the pull request from the event (by number on dispatch, by head branch on `workflow_run` restricted to open same-repository pull requests whose head is the commit, from the payload on `pull_request_target`), fetches the head SHA as objects, decides, and runs sweep, assessor, controller, and publish, writing the reason and counts to the step summary.
  3. `pr-risk.yml` declares the triggers from KTD2, a cheap `if` for same-repository heads, per-pull-request concurrency with `cancel-in-progress: false`, permissions `contents: read`, `pull-requests: write`, `checks: read`, `actions: read`, Node 24, `yarn install --immutable` from the base checkout, the assess step with `OPENAI_API_KEY_PR_RISK` and `OPENAI_API_KEY`, `actions/upload-artifact` for the matrix, and a job timeout.
  4. On `synchronize` the entry point runs only the stale transition.
...
- **Test scenarios:**
  ...
  - State `running` for the same head: `skip` with reason in progress; with `force`: `assess`.
...
- **Verification:** Tests pass; `gh workflow run pr-risk.yml -f pr=<n> -f dry_run=true` on a recent Copilot pull request prints the decision, the manifest counts, the route, and the rendered comment without writing; a second dispatch without `dry_run` creates the comment and labels and uploads the artifact; a push to that pull request marks it stale.
...
- **Approach:**
  1. `view.ts` reads the state from the sticky comment, downloads the artifact named there with the caller's token, refuses when the pull request head differs from the assessed head, and refuses when the token's login is not among the requested reviewers unless `--as-any-reviewer` is passed for the local case.
  2. `selectFiles.ts` returns rows whose file score is below the threshold from the policy or a `--threshold` flag, keyed by the path the pull request shows.
...
- **Files:**
  ...
  - `.github/instructions/pr-risk/samples.jsonl` (create: at least the transcript's seven shape cases mapped to real pull requests, with `pr`, `headSha`, `baseSha`, `expectedRoute`, `note`)
...
- **Approach:**
  ...
  2. Report route agreement, every sample whose route was `skip-eligible` against an expected `human`, and the per-sample cost; write the assessments to a local directory for inspection.
...
- **Test scenarios:**
  - Every sample has a valid pull request number, two 40-character SHAs, and an expected route in the allowed set.
...
| Real assessment, no writes | `gh workflow run pr-risk.yml -f pr=<n> -f dry_run=true` | U7 |
| Live shadow run | dispatch without `dry_run` on one Copilot pull request; comment, labels, and artifact appear; a push marks it stale | U7 |
| Auto-view | local command as a requested reviewer marks the expected files in that reviewer's session | U8 |
| Fixture routes | `yarn workspace em-pr-risk evaluate` reports every expected route | U9 |
...
- A dispatched shadow run on a real Copilot pull request publishes the comment, the labels, and the artifact; a push marks it stale; a re-dispatch with `force` reassesses.
- The evaluation harness runs over the samples and its report has been read.
