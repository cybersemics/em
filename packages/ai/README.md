...
Live model evaluations live in `src/evals/` and run together from the repository root with `yarn test:evals`. The Generate Emoji evaluation requires at least two matches among the ten generated results for each semantic category from issue #4400. The Organize Thought evaluation checks that the model groups related items into new categories, splits a compound thought without duplicating an existing item, preserves existing nesting (including descendants that move with a parent), reclassifies mixed existing categories instead of creating a leftover bucket, and omits context-only thoughts. Concurrent cases retry failures up to twice to accommodate model nondeterminism. Evaluations require the corresponding service key or the shared fallback in `.env.local` and are intentionally excluded from the default deterministic test suite.
