...
- `POST /ai/organizeThought` - Reorganizes a numbered indented outline of thoughts. Lines marked `[n]` are reorganizable; `[]` lines are context-only (the parent or unselected siblings). The model returns the final state as an indented outline with `[n]` for existing thoughts and `[new]` for new thoughts. The route validates and converts that text before responding with `{ "outline": [{ "id": "1" | null, "text": "..." | null, "children": [] }] }`, or `{ "error": "..." }` on failure. The client appends `/organizeThought` to `VITE_AI_URL`.
...
Live model evaluations live in `src/evals/` and run together from the repository root with `yarn test:evals`. The Generate Emoji evaluation requires at least two matches among the ten generated results for each semantic category from issue #4400. The Organize Thought evaluation asserts directly against the model's indented textual final state, covering semantic groups, compound splitting without duplication, preserved nesting, reclassification into existing categories, and omission of context-only thoughts. Concurrent cases retry failures up to twice to accommodate model nondeterminism. Evaluations require the corresponding service key or the shared fallback in `.env.local` and are intentionally excluded from the default deterministic test suite.
