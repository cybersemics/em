...
const migrateToDexie = async () => {
  ...
  const contextIndexPromises = Object.keys(contextIndexes).map(key => {
    ...
    console.log('-----', contextIndexes[key])
  ...
  const tis = await dbOperations.getThoughtIndexes()
  const cis = await dbOperations.getContextIndexes()
...
export const dbOperations = {
  updateThoughtIndex: async (id, thought) => {
    console.log({ id, ...thought })
    return db.thoughtIndex.put({ id, ...thought })
  },
