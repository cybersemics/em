...
const migrateToDexie = async () => {
  ...
  const helperPromises = (async () => db.helpers.put({ id: 'EM', ...{ helpersValuesToUpdate } }))()
  ...
  const migratedItems = await Promise.all([...helperPromises, ...thoughtIndexPromises, ...contextIndexPromises])
  const x = await dbOperations.getHelpers()
  console.log(migratedItems.length, x)
