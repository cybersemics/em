...
export const curryReducer = (<A extends any[]>(reducer: (...args: A) => State, arity?: number) => {
  ...
  const wrapper = (...args: Parameters<typeof curry>) => {
    ...
    Object.defineProperty(curried, '__curriedAction', { value: { type: reducer.name, ...args[0] } })
    return curried
  }

  // set __curriedAction on the wrapper so that reducers with no arguments can be dispatched by passing the function reference
  Object.defineProperty(wrapper, '__curriedAction', { value: { type: reducer.name } })
...
const reducerThunk: Middleware<any, State, Dispatch> = () => next => action =>
  next(
    // dispatch curried reducers using the payload stored in the __curriedAction property
    action.__curriedAction ||
