...
it('ignores navigation until the current transition finishes', () => {
  const { result } = renderHook(useCommandUniverseNavigator, { wrapper: OpenProvider })
  act(() => result.current.open('detail', { command }))
  const transition = result.current.transition
  act(() => {
    result.current.open('detail', { command: { ...command, label: 'ignored' } })
    result.current.back()
    result.current.forward()
  })
  expect(result.current.entries.map(entry => entry.page.pageId)).toEqual(['grid', 'detail'])
