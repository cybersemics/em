 ...
 * Groups are kept in difficulty order, so consumers can render a level heading whenever `level` changes.
 * Only commands executable on the current platform are included.
 */
const commandsGroupedByType: CommandGroup[] = COMMAND_GROUPS.flatMap(level =>
  level.groups
    .map(group => ({
      title: group.title,
      level: level.title,
 ...
 * Only commands executable on the current platform are included.
 */
const commandsSortedByLabel: Command[] = COMMAND_GROUPS.flatMap(level => level.groups.flatMap(group => group.commands))
  .map(commandById)
