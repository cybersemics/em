...
const useFilteredCommands = (
  ...
  const getPossibleCommandsSorted = useCallback(
    (search: string) => {
    ...
    [gestureInProgress, sortActiveCommandsFirst, recentCommands, store, platformCommandsOnly],
  )

  const possibleCommandsSorted = useMemo(() => getPossibleCommandsSorted(search), [getPossibleCommandsSorted, search])
  ...
  return { commands: possibleCommandsSorted, getCommandsSorted: getPossibleCommandsSorted }
}
