...
const chain = (state: State, contextChain: Path[], path: Path) => {
  ...
  const i = path.findIndex(child => pluralize(child.value) === pluralize(pivot.value))
  ...
  const appendedThoughtInContext = contexts.find(child => pluralize(head(child.context)) === pluralize(append[0].value))
