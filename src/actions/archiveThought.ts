  ...
  return reducerFlow([
    ...(isDeletable
      ...
      : [
          ...
          (state: State) => {
            const archive = findAnyChild(state, head(pathParent), equalThoughtValue('=archive'))!
            return moveThought(
              state,
              {
                oldPath: path,
                newPath: appendToPath(parentOf(path), archive.id, head(path)),
                offset,
...
export default command(archiveThought)
