...
it('prepends the first generated emoji and sends the named AI request', async () => {
  ...
  mockFetch.mockResolvedValueOnce({ json: () => Promise.resolve({ emojis }) })
...
it('regenerates from an edited concept and replaces its previous generated prefix', async () => {
  ...
  const updatedEmojis = ['🦊', '🐾', '🐕', '🐶', '🦮', '🦴', '🐕‍🦺', '🐩', '🐺', '🏠']
  mockFetch
    .mockResolvedValueOnce({ json: () => Promise.resolve({ emojis }) })
  ...
  expect(mockFetch).toHaveBeenLastCalledWith(
    ...
    expect.objectContaining({
      body: JSON.stringify({ value: 'Dogs' }),
    }),
  )
  expect(exportContext(store.getState(), [HOME_TOKEN], 'text/plain')).toBe(`- ${HOME_TOKEN}
  - 🦊 Dogs`)
...
it('does not overwrite an edit made while inference is pending', async () => {
  ...
  await dispatch(editThoughtByContext(['Dog...'], 'Dogs'))
  await act(async () => {
    resolveAiRequest({ json: () => Promise.resolve({ emojis }) })
  })
...
it('generates emoji for every selected thought within one undo step', async () => {
  acknowledgeAiDisclosure()
  mockFetch.mockResolvedValueOnce({ json: () => Promise.resolve({ emojis }) }).mockResolvedValueOnce({
