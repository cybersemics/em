...
/** Finds all the subthoughts of a thought. Returns the thought-container of each. Use findAllByLabelText([CHILD], 'thought') to select a child thought or use findSubthoughts(CHILD) to find child subthoughts. */
export const findSubthoughts = async (value?: HTMLElement | string | null): Promise<HTMLElement[]> => {
  ...
  const container = getClosestByLabel(thought, 'thought-container')!
  ...
  const children = Array.from(subthoughts.childNodes).filter(
    child => (child as HTMLElement)?.getAttribute('aria-label') === 'thought-container',
  )
