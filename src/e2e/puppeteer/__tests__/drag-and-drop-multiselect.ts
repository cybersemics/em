...
`)
    ...
    // When the alert is dismissed the element is absent, so $eval rejects; fall back to '' (not null)
    // so that .not.toContain has a valid string to operate on in the success case.
