...
/** Tap an element by selector at its center.*/
const tapElementBySelector = async (selector: string) => {
  ...
  return tap(el, { horizontalTapLine: 'center' })
}
...
it('Can change the background color of a note that already has the same background color applied to part of its text, then change the text color', async () => {
  ...
  await tapElementBySelector('[data-testid="toolbar-icon"][aria-label="Note"]')
