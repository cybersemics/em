...
const baseConfig = {
  ...
  // This is also the most sessions a BrowserStack config waits for in onPrepare (browserStackLauncherHooks)
  // — fewer when --spec names fewer files, or when the config lowers maxInstances as the Android one does —
  // so the pool (and its queue) is known to have room before any worker starts.
  maxInstances: 2,
