...
const DepthBar = ({ numDescendantCharacters, showContexts, thoughtsLive }: DepthBarProps) => (
  <span>
    {numDescendantCharacters >= 16 ? (
      <Modal
        id='depthBar'
    ...
    {(showContexts ? parentOf(thoughtsLive) : thoughtsLive) && numDescendantCharacters > 0 ? (
      <span
        className={classNames({
          'depth-bar': true,
          'has-other-contexts':
            thoughtsLive.length > 1 &&
            getContexts(store.getState(), head(showContexts ? parentOf(thoughtsLive) : thoughtsLive)).length > 1,
        })}
        style={{ width: Math.log(numDescendantCharacters) + 2 }}
      />
    ) : null}
  </span>
)
