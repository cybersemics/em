...
const TutorialStepSubThought = ({ cursor }) => <Fragment>
  ...
  {!cursor ? <p>{isTouch ? 'Tap' : 'Click'} a thought to select it.</p> : <p>{isTouch ? 'Trace the line below' : `${cursor && headValue(cursor) === '' ? 'Then h' : 'H'}old the ${isMac ? 'Command' : 'Ctrl'} key and hit the Enter key`}.</p>}
</Fragment>
