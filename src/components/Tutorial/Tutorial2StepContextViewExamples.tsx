import { tutorialBulletRecipe } from '../../../styled-system/recipes'

// eslint-disable-next-line jsdoc/require-jsdoc
const Tutorial2StepContextViewExamples = () => (
  <>
    <p>
      Here are some real-world examples of using contexts in <b>em</b>:
    </p>
    <ul>
      <li className={tutorialBulletRecipe()}>View all thoughts related to a particular person, place, or thing.</li>
      <li className={tutorialBulletRecipe()}>Keep track of quotations from different sources.</li>
      <li className={tutorialBulletRecipe()}>
        Create a link on the home screen to a deeply nested subthought for easy access.
      </li>
    </ul>
    <p>
      The more thoughts you add to <b>em</b>, the more useful this feature will become.
    </p>
  </>
)

export default Tutorial2StepContextViewExamples
