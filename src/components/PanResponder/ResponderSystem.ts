 ...
 *
 * Faithful TypeScript port of react-native-web's ResponderSystem.js.
 *
...
interface ActiveResponderInstance {
  id: ResponderId
  idPath: number[]
  node: any
}

interface EmptyResponderInstance {
  id: null
  idPath: null
  node: null
}
...
export interface ResponderConfig {
  ...
  // On scroll, should this element become the responder? Do not bubble
...
const emptyObject: ResponderConfig = {}

/* ------------ IMPLEMENTATION ------------ */

const startRegistration = ['onStartShouldSetResponderCapture', 'onStartShouldSetResponder', { bubbles: true }] as const
const moveRegistration = ['onMoveShouldSetResponderCapture', 'onMoveShouldSetResponder', { bubbles: true }] as const
...
const shouldSetResponderEvents: Record<string, readonly [string, string, { bubbles: boolean }]> = {
...
const emptyResponder: EmptyResponderInstance = { id: null, idPath: null, node: null }
...
/** Changes the current responder to the given instance. */
function changeCurrentResponder(responder: ResponderInstance): void {
  currentResponder = responder
}

/** Gets the responder config for a given responder ID. */
 ...
 * callbacks.
 */
function eventListener(domEvent: any): void {
  ...
  if (isStartEvent || isMoveEvent || (isScrollEvent && trackedTouchCount > 0)) {
    ...
    if (eventPaths != null) {
      ...
      if (wantsResponder != null) {
        // Sets responder if none exists, or negotiates with existing responder.
    ...
    // Move
      ...
      if (isTerminateEvent) {
        ...
        if (eventType === 'contextmenu' || eventType === 'scroll' || eventType === 'selectionchange') {
          // Only call this function if it wasn't already called during negotiation.
          if (wasNegotiated) {
...
function findWantsResponder(
...
): ActiveResponderInstance | undefined {
  ...
  if (shouldSetCallbacks != null) {
    ...
    const check = function (id: number, node: any, callbackName: string): ActiveResponderInstance | undefined {
      const config = getResponderConfig(id)
      const shouldSetCallback = (config as any)[callbackName]
...
/** Attempts to transfer the responder to a new node. */
function attemptTransfer(responderEvent: ResponderEvent, wantsResponder: ActiveResponderInstance): void {
...
/** Registers native event listeners on document and window. */
...
/** Registers a node with the ResponderSystem. */
export function addNode(id: ResponderId, node: any, config: ResponderConfig): void {
  setResponderId(node, id)
  responderListenersMap.set(id, config)
}

/** Unregisters a node with the ResponderSystem. */
export function removeNode(id: ResponderId): void {
