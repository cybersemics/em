...
it('expands the pinned gesture and reveals its command without changing the pin', async () => {
  ...
  expect(screen.getByTestId('pinned-command')).toHaveAttribute('aria-expanded', 'true')
...
it('shows the shortcut for a command without a gesture and closes on Escape', async () => {
  await act(async () => {
  ...
  expect(screen.getByRole('dialog', { name: 'Gesture for Settings' })).toHaveTextContent('Press Ctrl + , to activate.')

  await user.keyboard('{Escape}')
  expect(screen.getByTestId('pinned-command')).toHaveAttribute('aria-expanded', 'false')
