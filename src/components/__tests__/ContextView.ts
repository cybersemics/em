...
describe('render', () => {
  ...
  it('change bullet to no fill', async () => {
    ...
    // In the context view the bullet is rendered as an unfilled triangle.
...
describe('editing', () => {
  it('edit a context', async () => {
    ...
    // the caret is at the start of the thought, so the typed character is prepended
    await user.type(thoughtA, 'z')

    await act(vi.runAllTimersAsync)

    // the edited context is the thought itself, so it is renamed both in the context view and in the tree
    expect(await findAllThoughtsByText('za')).toHaveLength(2)
    expect(await queryThoughtByText('a')).toBeNull()

    // the other context is unaffected
