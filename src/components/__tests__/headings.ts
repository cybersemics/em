...
it('headings should set font weight', async () => {
  ...
  const thought1 = (await findThoughtByText('My Heading 1'))?.closest('[aria-label="thought-container"]')
  expect(thought1).toHaveStyle({ fontWeight: 700 })

  const thought2 = (await findThoughtByText('My Heading 2'))?.closest('[aria-label="thought-container"]')
  expect(thought2).toHaveStyle({ fontWeight: 700 })

  const thought3 = (await findThoughtByText('My Heading 3'))?.closest('[aria-label="thought-container"]')
  expect(thought3).toHaveStyle({ fontWeight: 700 })

  const thought4 = (await findThoughtByText('My Heading 4'))?.closest('[aria-label="thought-container"]')
  expect(thought4).toHaveStyle({ fontWeight: 600 })

  const thought5 = (await findThoughtByText('My Heading 5'))?.closest('[aria-label="thought-container"]')
