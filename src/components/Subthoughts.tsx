...
const drop = (props: SubthoughtsProps, monitor: DropTargetMonitor) => {
  ...
  const isAtBottom = equalPath(rootedParentOf(thoughtsFrom), thoughtsTo)
...
export const SubthoughtsComponent = ({
  ...
  const topPinned = children.filter(child => {
    const value = showContexts ? head((child as ThoughtContext).context) : (child as Child).value
    const childContext = [...pathToContext(unroot(simplePath)), value]
  ...
  const filteredChildrenUnpinned = children.filter(child => {
  ...
  const filteredChildren = sortPreference === 'Alphabetical' ? [
    ...topPinned || [],
    ...filteredChildrenUnpinned,
    ...bottomPinned || []
  ] : filteredChildrenUnpinned
