...
/** New Subthought icon. */
const NewSubthoughtIcon = ({ fill, size = 20, style = {}, className }: IconType) => {
  const newSize = size * ICON_SCALING_FACTOR
  const strokeColor = style.fill || fill || token('colors.fg')
  const fillColor = style.fill || fill || token('colors.fg') // Assuming the same color for fill if needed
