...
/** Generates the placeholder text for the thought. Automatically changes from 'Add a thought' to 'This is an empty thought' after a short delay. Handles the special case where the cursor is in a clear state due to the clearThought command. Empty thoughts show 'Generating Thought' while Generate Thought is in flight. */
const usePlaceholder = ({ isEditing, path, simplePath }: { isEditing: boolean; path: Path; simplePath: SimplePath }) =>
  useSelector(state => {
    ...
    const { value } = thought
    // Display-only: the stored value stays empty while Generate Thought is in flight.
    if (thought.generating && !value) return 'Generating Thought'
