...
const toggleSpitViewSVG = ({ fill = 'white', size = 20, id }) => <svg id={id} className="icon" width={size} height={size} viewBox="0 0 110 110">
  <path
...
export default {
  ...
  description: 'Display the 2 view panel',
  gesture: 'ru',
  // keyboard: { key: 'p', shift: true, meta: true },
  svg: toggleSpitViewSVG,
  exec: () => {
    const state = store.getState()
    if (state) {
      store.dispatch({ type: 'toggleSplitView' })
      const showSplitView = state.showSplitView
      store.dispatch({ type: 'toggleSplitView', value: !showSplitView })
    }
  }
}
