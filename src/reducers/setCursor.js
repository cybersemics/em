...
export default (
	state,
	{ thoughtsRanked, contextChain = [], cursorHistoryClear, cursorHistoryPop, replaceContextViews, editing }
) => {
	const thoughtsResolved =
		contextChain.length > 0 ? chain(contextChain, thoughtsRanked, state.thoughtIndex) : thoughtsRanked;
	...
	setTimeout(() => {
		...
		if (thoughtsResolved) {
			...
			localForage
				.setItem('cursor', hashContextUrl(pathToContext(thoughtsResolved), { contextViews: newContextViews }))
				.catch((err) => {
					throw new Error(err);
				});
		} else {
	...
	const tutorialNext =
		...
		(tutorialStep === TUTORIAL2_STEP_CONTEXT_VIEW_SELECT &&
			thoughtsResolved &&
			thoughtsResolved.length >= 1 &&
			headValue(thoughtsResolved).toLowerCase().replace(/"/g, '') ===
	...
	return equalPath(thoughtsResolved, state.cursor) && state.contextViews === newContextViews
		...
		: {
				...
				cursorHistory: cursorHistoryClear
					? []
					: cursorHistoryPop
						? state.cursorHistory.slice(0, state.cursorHistory.length - 1)
						: state.cursorHistory,
