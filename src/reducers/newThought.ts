...
const newThought = (state: State, payload: NewThoughtPayload | string) => {
  ...
  const newRank = (showContextsParent && !insertNewSubthought) || (showContexts && insertNewSubthought)
    ...
    : insertBefore
      ? insertNewSubthought || !thoughtsRanked ? getPrevRank(state, thoughts) : getRankBefore(state, thoughtsRanked)
      : insertNewSubthought || !thoughtsRanked ? getNextRank(state, thoughts) : getRankAfter(state, thoughtsRanked)
