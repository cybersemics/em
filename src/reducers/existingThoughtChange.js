...
// constants
import { EM_TOKEN, RECENTLY_EDITED_THOUGHTS_LIMIT } from '../constants.js';
...
export default (
	...
	const thoughtsRankedLiveOld = showContexts
		? contextOf(contextOf(thoughtsRanked))
				.concat({ value: oldValue, rank: headRank(contextOf(thoughtsRanked)) })
				.concat(head(thoughtsRanked))
	...
	const cursorNew =
		...
		state.cursor.map(
			(thought) =>
				thought.value === oldValue && thought.rank === rankInContext
	...
	const newPath = oldPath
		.slice(0, oldPath.length - 1)
		.concat({ value: newValue, rank: oldPath.slice(oldPath.length - 1)[0].rank });
  ...
  */
	...
	const recentlyEdited = reverse(
		sortBy(
			[ ...state.recentlyEdited ]
				.filter((recentlyEditedThought) => !equalPath(recentlyEditedThought.path, oldPath))
				.map(
					(recentlyEditedThought) =>
						subsetThoughts(recentlyEditedThought.path, oldPath)
							? Object.assign({}, recentlyEditedThought, {
									path: newPath.concat(recentlyEditedThought.path.slice(newPath.length))
								})
							: recentlyEditedThought
				)
				...
				.map((recentlyEditedThought) => {
					...
					return subcontextIndex > -1 && timeDiffInSec <= 7200
						? { path: newPath.slice(0, subcontextIndex + 1), lastUpdated: timestamp() }
						: recentlyEditedThought;
				})
	...
	const isThoughtOldSubthoughtless = () =>
		getThoughtsRanked([ { value: oldValue, rank } ], state.thoughtIndex, state.contextIndex).length < 2;
	...
	const thoughtNew =
		thoughtOld.contexts.length > 0
			? addContext(
					...
					context,
					showContexts ? headRank(rootedContextOf(thoughtsRankedLiveOld)) : rank
				)
			: newThoughtWithoutContext;
	...
	if (showContexts) {
		thoughtParentNew = Object.assign({}, thoughtParentOld, {
			contexts: removeContext(
				thoughtParentOld,
				contextOf(pathToContext(thoughtsRankedLiveOld)),
				rank
			).contexts.concat({
				context: thoughtsNew,
				rank
	...
	const contextParentEncoded = hashContext(
		rootedContextOf(showContexts ? context : pathToContext(thoughtsRankedLiveOld))
	);

	const thoughtParentSubthoughts = showContexts
		? (state.contextIndex[contextParentEncoded] || [])
				.filter(
					(child) =>
						(newOldThought ||
		...
		return getThoughtsRanked(thoughtsRanked, state.thoughtIndex, state.contextIndex).reduce((accum, child) => {
			...
			const accumNew = {
				...
				[hashedKey]: {
					...
					contextsOld: ((accumRecursive[hashedKey] || {}).contextsOld || [])
						.concat([ pathToContext(thoughtsRanked) ]),
	...
	setTimeout(() => {
		...
		if (local) {
			...
			localForage
				.setItem('cursor', hashContextUrl(pathToContext(cursorNew), { contextViews: contextViewsNew }))
				.catch((err) => {
					throw new Error(err);
				});
