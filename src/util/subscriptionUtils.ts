...
export const getSubscriptionUtils = ({ getState, dispatch }: Store<State, Action<string>>) => ({
  ...
  shouldIncludeUpdate: (thoughtOrContext?: Parent | Lexeme, updateType?: SessionType) => {
    ...
    else if ('contexts' in thoughtOrContext && !thoughtOrContext.contexts.find(c => isContextVisible(state, c.context))) return false
  ...
  getUpdatedObject: <T extends Index>(original: T, pathUpdates: Index) => Object.keys(pathUpdates).reduce((acc, key) => {
