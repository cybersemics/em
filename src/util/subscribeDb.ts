...
const getDbChangeHandlers = ({ getContextLocal, getThoughtLocal, getUpdatedObject, shouldIncludeUpdate }: ReturnType<typeof getSubscriptionUtils>) => ({
  [dbChangeType.created]: (change: IDatabaseChange) => {
    ...
    return {
      thoughtIndexUpdates: table === dbTables.thoughtIndex && shouldIncludeUpdate(obj, SessionType.LOCAL) ? { [key as string]: obj as Lexeme } : {},
      contextIndexUpdates: table === dbTables.contextIndex && shouldIncludeUpdate(obj, SessionType.LOCAL) ? { [key as string]: obj as Parent } : {}
    }
  },
  ...
  [dbChangeType.deleted]: (change: IDatabaseChange) => {
    ...
    return {
      thoughtIndexUpdates: table === dbTables.thoughtIndex && oldObj && oldObj.id && shouldIncludeUpdate(oldObj, SessionType.LOCAL) && getThoughtLocal(key) ? { [key as string]: null } : {},
