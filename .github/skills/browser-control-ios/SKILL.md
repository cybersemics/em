...
The session drives a pre-built **server-mode** build of **em**'s Capacitor app. The IPA for this build is already uploaded to BrowserStack under the `custom_id` **`em-server-mode`** (override with `EM_IOS_APP`), so day-to-day web changes need no rebuild. Reference it by `custom_id`; do **not** rebuild the native app per run. If bring-up fails with an app-not-found error (in `/tmp/em-ios-bringup.log`), the pre-warmed app has lapsed (BrowserStack deletes apps 30 days after last use) — **escalate to the user** to rebuild and re-upload it with `yarn build:ios:browserstack` (`scripts/build-em-browserstack-ipa.sh`; needs a Mac with Xcode signing set up, so agents cannot do it themselves).

> **App ids are per-account.** A bare `custom_id` resolves only against apps uploaded by the account whose credentials created the session; apps uploaded by other accounts in the org must be referenced by their `shareable_id` (`<uploader>/<custom_id>`, e.g. `EM_IOS_APP='faizsaleem_ybmLal/em-server-mode'`). `GET https://api-cloud.browserstack.com/app-automate/recent_group_apps` (basic auth) lists every upload in the group with both ids. Keep to the pinned `em-server-mode` upload — do not push test builds over that `custom_id`, since bare-id resolution always picks the uploader's newest.

> **The IPA must come from `yarn build:ios:browserstack`, not a plain Xcode Release archive.** The webview only loads if the build (1) bakes `server.url: https://bs-local.com:3000` (an inline `CAPACITOR_SERVER_URL` that loses to the `.env` cascade bakes `localhost` instead — the app then black-screens without ever touching the tunnel) and (2) is a **Debug** configuration, because `DevServerCertPlugin` (`ios/App/App/DevServerViewController.swift`), which accepts the dev server's self-signed cert at the WKWebView TLS challenge, is compiled out of Release builds — a Release IPA connects through the tunnel once, rejects the cert, and renders a blank webview. The build script handles both; both failure modes were shipped and diagnosed in Aug 2026, so suspect the IPA before suspecting the tunnel.

### Heartbeat

`bringup.sh` starts the heartbeat **automatically** once the session id lands (you don't run it by hand). It keeps the BrowserStack session alive during long agent sessions — pings the hub every 90s (a 405 from the native context counts as alive: the session answered, and 405-answered pings verifiably reset the idle timer), logs to `/tmp/heartbeat-<id>.log`, self-exits after 3 consecutive failures, and appends BrowserStack's session post-mortem on give-up (the only post-hoc signal for why a session died). The session id is in `/tmp/em-bs-session.txt`, which the wdio-MCP shim and e2e bridge both read.

## Step 2: Connect a driver and land in the webview context

The app launches and its WKWebView **auto-loads the dev server on launch** (its baked `server.url`, `https://bs-local.com:3000`, reaches the runner's HTTPS dev server through the BrowserStack Local tunnel) — there is **no `navigate` step**. The self-signed dev cert is accepted by `DevServerViewController` inside the app, so — unlike the Safari E2E tests, which need a cloudflared CA-signed cert — no public tunnel is required here. (The dev server's token gate only guards the Cloudflare tunnel's public hostnames; `bs-local.com` is not one, so the app loads without a `?__token` param.) If the webview never mounts (black screen, no `WEBVIEW_*` context, Step 2 wait times out), suspect a bad IPA — see **App binary** above. To tell the two bad-IPA modes apart, watch the dev server's port while relaunching the app (`mobile: terminateApp` then `mobile: activateApp` via `POST /execute/sync` — `mobile:` scripts work in the native context): a connection that appears and drops means the cert plugin is missing (Release build); no connection at all means the baked `server.url` is wrong. The app attempts its `server.url` exactly once per launch, so only a relaunch (not waiting) reproduces the attempt.
