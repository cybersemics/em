...
**Why actuation must use a helper — the trap that bites.** Some of em's controls are wired to **touch events only** when `isTouch` is true, so a raw mouse click — `page.click`, an MCP `click`, `elementHandle.click()` — **silently does nothing** on a touch-emulated page: no error, no effect, and you misread it as "the button is broken." The **`click` helper taps correctly per platform** (`page.tap` on mobile, `page.click` on desktop). Never actuate an em control with a raw click; use the helper.

Which controls: **`ToolbarButton` and the `ColorPicker` swatches** bind `onClick` only when `!isTouch` and `onTouchEnd` only when `isTouch` (`src/components/ToolbarButton.tsx`, `src/components/ColorPicker.tsx`), so under emulation a mouse click reaches neither. This is the failure you will actually hit, and it covers the whole toolbar and every dropdown value inside it. `fastClick` itself is **not** the culprit — it returns an unconditional `onClick`, so the ~50 components that use it do respond to a raw click even under emulation. Two reasons to still route everything through the helper rather than tracking which is which: `fastClick`'s `tapDown` is `isTouch`-gated too (a raw click never fires it, so long-press and the toolbar's swipe-vs-tap bookkeeping don't run), and a repro built from helpers transfers into a test for free.
