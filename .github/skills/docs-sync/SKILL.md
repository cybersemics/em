...
| [`debug-log.md`](../../docs/debug-log.md) | `src/util/debugLog.ts`, `src/redux-middleware/loggerMiddleware.ts`, and the `DebugLogging` controls in `src/components/modals/Settings.tsx` | The rolling buffer, its persistence, hydration, the frame heartbeat, when logging auto-enables, or the format of a shared log changes; or an entry type is added, renamed, or retired |
...
| [`commands.md`](../../docs/commands.md) | `src/commands/**`, `src/commands.ts`, and the undo history: `src/redux-enhancers/undoRedoEnhancer.ts`, `src/actions/undo.ts`, `src/actions/redo.ts`, `src/selectors/undoSteps.ts`, `src/selectors/stepsToReproduce.ts`, `src/components/UndoSlider.tsx` | You add, remove, or change a command, its shortcut, or the command system itself; or the undo history, its grouping into steps, or the undo slider changes |
...
| [`testing.md`](../../docs/testing.md) | `src/e2e/**`, `src/test-helpers/**`, `src/setupTests.ts`, `src/vite-middleware/**` and the dev-server wiring in `vite.config.ts` (the tunnel token gate), and the CI workflows under `.github/workflows/` that the agent rows below do not claim | Helper contracts, test patterns, platforms, or runners change, a workflow's triggers, path filters, concurrency, or reported checks change, or the tunnel gate's routes or gating rule change |
| [`agents/*.md`](../../docs/agents/) | `.github/skills/**`, `.github/agents/**`, `.github/instructions/**`, `.github/copilot-instructions.md`, `AGENTS.md`, `.agents/**`, `.github/workflows/copilot-setup-steps.yml`, `.github/workflows/tdd.yml`, the `scripts/` the agent depends on (`*.mjs`, `*.sh`) | Anything about how any agent works changes — a skill, either entry point, which skills are shared, the environment, the MCP wiring |
