...
| [`testing.md`](../../docs/testing.md) | `src/e2e/**`, `src/test-helpers/**`, `src/setupTests.js`, and the CI workflows under `.github/workflows/` that the agent rows below do not claim | Helper contracts, test patterns, platforms, or runners change, or a workflow's triggers, path filters, concurrency, or reported checks change |
