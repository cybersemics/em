...
- **The trap:** `ToolbarButton` and the `ColorPicker` swatches bind `onClick` only when `!isTouch`, so a **raw mouse click silently no-ops** on a touch-emulated page — no error, you just misread it as "the button doesn't work." That covers the whole toolbar and every dropdown value in it. (`fastClick` controls do respond to a raw click; its `onClick` is unconditional. Its `tapDown` is not, so a raw click still skips long-press and the toolbar's swipe-vs-tap bookkeeping.) The **`click` helper** taps right per platform (`page.tap` mobile / `page.click` desktop). A tap is **not** "mechanical" just because it's a tap — if it's em's own UI, use the helper. Check the catalog before assuming otherwise.
