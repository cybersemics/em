...
**👆 Multiselect** — **Secondary; yields to any primary subsystem.** It owns general multiselect behaviour itself: selecting, deselecting, extending a selection, and where the cursor sits before or after a multiselect command runs. **The Command Center opening and closing belongs here too**: it opens when thoughts are selected and closes when they are deselected, one to one, so an issue about it opening or closing when it should not — or failing to — is a selection-state issue. Its buttons, styling, and animation are not. When a multiselect interaction breaks some other feature — drag and drop, context view, sorting — that feature owns the issue; multiselect is then how the bug was reached, not what it is about.
...
**🫧 Liminal UI** — **Secondary; yields to any primary subsystem.** The Liminal UI design work itself: the Command Universe, the gesture menu, dialogs, glass styling, notification surfaces, and the component refactors that support them. A functional bug merely reached through one of those surfaces belongs to the subsystem that broke. Note that the Command Center's open and closed state is 👆 Multiselect, not this; only its buttons, styling, and animation are Liminal UI's.
